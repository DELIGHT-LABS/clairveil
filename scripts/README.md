...
- `session-3a-prepare-artifact-evidence.sh`: from a clean tree, prepares previous/current JoinSplit artifact directories outside the repository for source-bound rotation evidence.
- `session-3a-validation-evidence.sh`: runs the exact Session 3A rotation, fresh-genesis, and regression evidence gates using supplied or freshly prepared artifact sets.
