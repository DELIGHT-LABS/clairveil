...
- `release-pack.sh`: creates the downstream handoff tarball and external sha256 file under `dist/`, with deterministic Git-derived file and canonical directory/metadata modes independent of caller umask.
- `release-pack-verify.sh`: verifies canonical/safe raw tar members, the exact selected Git file set, canonical manifest, checksums, required files, and every packed Git blob with its exact raw and extracted Git-derived permission mode. Explicit archives and checksums must already exist, are never regenerated, and require an exact lowercase 40-character `RELEASE_PACK_EXPECTED_COMMIT`.
