...
- `release-pack-verify.sh`: verifies canonical/safe tar members, the exact selected Git file set, canonical manifest, checksums, required files, and every packed Git blob with its exact Git-derived permission mode. Explicit archives require an exact lowercase 40-character `RELEASE_PACK_EXPECTED_COMMIT`.
