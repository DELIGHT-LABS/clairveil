...
- `docs-check.sh`: CommonMark/GFM AST로 tracked/new working-tree Markdown link와 fragment, top-level 문서 언어 pair, 완전한 docs/plan index, `HEAD`에서 도달 가능한 exact-SemVer annotated commit tag와 실제 changelog heading, 문서 위치, release-pack manifest, packed-link closure를 검증합니다.
