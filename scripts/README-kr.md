...
- `release-pack-verify.sh`: canonical/safe raw tar member, exact selected Git file set, canonical manifest, checksum, 필수 파일, 모든 packed Git blob과 raw/extracted Git-derived exact permission mode를 검증합니다. Explicit archive/checksum은 미리 존재해야 하고 재생성되지 않으며 exact lowercase 40-character `RELEASE_PACK_EXPECTED_COMMIT`이 필요합니다.
