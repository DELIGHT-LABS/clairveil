...
- `release-pack-verify.sh`: canonical/safe tar member, exact selected Git file set, canonical manifest, checksum, 필수 파일, 모든 packed Git blob과 Git-derived exact permission mode를 검증합니다. Explicit archive에는 exact lowercase 40-character `RELEASE_PACK_EXPECTED_COMMIT`이 필요합니다.
