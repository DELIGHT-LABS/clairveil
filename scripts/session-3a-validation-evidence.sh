...
if [[ -z "$current_dir" && -z "$previous_dir" ]]; then
	...
	fail "set both artifact directory variables or neither for clean source-bound regeneration"
fi
...
run_exact_test() {
	...
	local -a test_command=(go test "$package" -run "^${test_name}$" -count=1 -p=1 -v)
	if [[ -n "$gate_env" ]]; then
		test_command=(env "$gate_env=1" "${test_command[@]}")
	fi
	if ! (
		cd "$repo_root"
		"${test_command[@]}" 2>&1 | tee "$test_output"
	); then
...
run_exact_test \
	./x/privacy/zk \
	TestJoinSplitArtifactRotationSnapshotValidation
