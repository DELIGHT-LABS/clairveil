...
func preparedCollectedDeposit(t *testing.T) (CollectedAuditTx, [32]byte, privacycrypto.AuditSecretKey) {
	...
	prepared, err := Prepare(PrepareInput{
		...
		Kind:     auditfield.KindDeposit, Creator: creator, Amount: "7uclair", PublicAsset: assetBytes, Principal: creatorBytes,
		ExpiresAtUnix: 100, Outputs: []*privacyv2.OutputEffect{output},
		Plaintext: []auditfield.Field32{asset, auditfield.Field32FromUint64(7), auditfield.Field32FromUint64(2), auditfield.Field32FromUint64(3), auditfield.Field32FromUint64(4), auditfield.Field32FromUint64(5)},
	})
	require.NoError(t, err)
	defer prepared.Clear()
	envelope, err := prepared.EnvelopeBytes()
	require.NoError(t, err)
	message := &privacyv2.MsgDeposit{Creator: creator, Amount: "7uclair", Output: output, Proof: make([]byte, 164), ExpiresAtUnix: 100, Audit: &privacyv2.AuditAuthorization{KeyId: key.IDBytes(), Epoch: 1, Envelope: envelope}}
