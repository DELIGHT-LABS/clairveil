...
func ValidateBatchTransferSigningRequest(req BatchTransferSigningRequest) error {
	...
	var computedInputTotal privacyamount.Amount128
	...
	for i := range req.OrderedInputNullifiers {
		...
		computedInputTotal, err = computedInputTotal.Add(input.Amount)
		if err != nil {
			return fmt.Errorf("operation total exceeds uint128: %w", err)
		}
	}
	if privacytypes.Amount128BigInt(computedInputTotal).Cmp(req.InputTotal) != 0 {
	...
	var computedOutputTotal privacyamount.Amount128
	seenPayment := false
	seenChange := false
	seenPadding := false
	...
	for i := range req.OrderedOutputs {
		...
		computedOutputTotal, err = computedOutputTotal.Add(o.Amount)
		if err != nil {
			return fmt.Errorf("operation total exceeds uint128: %w", err)
		}
	...
	if computedOutputTotal != computedInputTotal {
...
func ValidatePreparedBatchTransferPayloadMetadataAt(p *PreparedBatchTransferPayload, now time.Time) error {
	...
	for i, in := range p.Inputs {
		...
		inputTotal, err = inputTotal.Add(in.Note.Amount)
		if err != nil {
			return fmt.Errorf("operation total exceeds uint128: %w", err)
		}
	...
	var outputTotal privacyamount.Amount128
	for i, out := range p.Outputs {
		...
		outputTotal, err = outputTotal.Add(out.Note.Amount)
		if err != nil {
			return fmt.Errorf("operation total exceeds uint128: %w", err)
		}
	}
