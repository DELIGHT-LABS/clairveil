...
func TestDepositWithFunderMatchesMsgServerDeposit(t *testing.T) {
	...
	require.Zero(t, publicBank.getBalanceCalls)
	require.Equal(t, 2, trustedBank.getBalanceCalls)
	require.Less(t, publicCtx.GasMeter().GasConsumed(), trustedCtx.GasMeter().GasConsumed())
