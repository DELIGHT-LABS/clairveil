...
Public capacity benchmark 하네스와 claim gate의 완료 범위는 [Clairveil Public Capacity Benchmark 완료 계획](clairveil-public-capacity-benchmark-followup-plan-kr.md)을 기준으로 합니다. 실제 TPS/운영 prover 처리량/사용자 체감 지연을 public claim으로 만들기 위해 남은 실행 작업은 [Clairveil Public Capacity Claim 실행 계획](clairveil-public-capacity-claim-execution-plan-kr.md)에서 별도로 관리합니다.
...
- `make privacy-proverd-load-bench`

`make privacy-proverd-bench`는 `httptest` 기반 in-process HTTP transport round trip입니다. JSON encode/decode, request/response validation, HTTP client/server overhead를 sequential/parallel benchmark로 고정하므로 공개 문구에서는 "prover HTTP transport benchmark"로 표기하고 production sidecar capacity로 쓰지 않습니다.

`make privacy-proverd-load-bench`는 이미 실행 중인 external `clairveil-proverd`를 대상으로 concurrency sweep, latency percentile, error/timeout rate, request/response byte, `/debug/vars` 기반 CPU/RSS/heap/goroutine telemetry를 structured summary로 기록합니다. 운영 prover 처리량 claim은 이 external load report와 saturation evidence를 기준으로 합니다.
...
- `cmd/clairveil-localnetload`와 `make privacy-localnet-tps-bench`가 tx metrics를 `chain_tps` structured summary로 변환해 submitted/accepted/included/successful tx/sec, failed tx rate, inclusion latency, gas metric을 기록합니다. Tx별 `success`가 명시되지 않은 입력은 성공으로 추정하지 않고 실패로 보수 계산합니다.
- 기본 temp workdir은 benchmark 종료 후 정리됩니다. 디버깅용 workdir을 남기려면 `KEEP_BENCH_WORK_DIR=1`을 사용합니다.

현재 `privacy-localnet-tps-bench` wrapper는 e2e smoke flow를 계측 가능한 TPS report로 승격하는 경로입니다. Public chain TPS claim을 만들기 위한 open-loop/batch 실행, target tx/sec sweep, reserve snapshot, chain config evidence, saturation profile 수집은 [Clairveil Public Capacity Claim 실행 계획](clairveil-public-capacity-claim-execution-plan-kr.md)의 Track B에서 관리합니다.
...
현재 상태: 완료. `cmd/clairveil-benchreport`가 commit, dirty state, Go/gnark/gnark-crypto version, OS/arch, CPU, active set id, optional manifest checksum, artifact descriptor completeness, 실제 artifact file checksum verification, result family/source files/run window를 기록합니다. 실제 metric 입력인 `-input`, `-benchmark-summaries`, `-tx-metrics`는 source provenance에 자동 포함됩니다. `make privacy-bench`, `make privacy-proverd-bench`, `make privacy-bench-localnet`는 output 파일을 만들기 전에 source commit과 dirty state를 캡처하고, untracked status를 파일 단위로 펼친 뒤 generated benchmark result family(`benchmarks/privacy-circuits`, `benchmarks/privacy-proverd`, `benchmarks/privacy-localnet` 등)의 untracked 생성물과 Makefile build가 루트에 만드는 정확한 바이너리 이름(`clairveild`, `clairveil-benchreport` 등)만 source dirty 계산에서 제외해 report artifact 생성 또는 release-check/build 선행 실행 자체가 `dirty: true`로 오인되지 않게 합니다. `benchmarks/` 아래의 tracked baseline, 문서 변경, tracked 바이너리 변경, 비슷한 이름의 임의 파일은 dirty로 잡혀야 합니다.
...
현재 상태: 하네스 구현 완료. `make privacy-proverd-bench`가 in-process HTTP transport sequential/parallel round trip benchmark를 생성합니다. `cmd/clairveil-proverload`와 `make privacy-proverd-load-bench`는 이미 실행 중인 external `clairveil-proverd`를 대상으로 fixture-backed HTTP load를 걸고, concurrency별 `requests/sec`, `latency_ms`, `error_rate`, `timeout_rate`, request/response byte summary를 `benchmarks/privacy-proverd-load` structured report로 생성합니다. `clairveil-proverd`의 `/debug/vars` endpoint와 `cmd/clairveil-proverload -telemetry-interval` 샘플링으로 `cpu_percent`, `rss_bytes`, `max_rss_bytes`, heap/goroutine metric도 같은 `prover_rps` row에 포함됩니다. Public RPS claim용 운영형 request pool, 10분 이상 sweep, saturation profile evidence 수집은 [Clairveil Public Capacity Claim 실행 계획](clairveil-public-capacity-claim-execution-plan-kr.md)의 Track A에서 관리합니다.
...
현재 상태: 하네스 구현 완료. `make privacy-bench-localnet`가 e2e smoke tx query에서 gas/success/height/timestamp metric을 추출하고 fee report, reserve invariant snapshot, localnet structured summary를 생성합니다. `cmd/clairveil-localnetload`와 `make privacy-localnet-tps-bench`는 tx metrics를 `chain_tps` row로 변환해 `submitted_tx/sec`, `accepted_tx/sec`, `included_tx/sec`, `successful_tx/sec`, `failed_tx_rate`, `inclusion_latency_ms`, `gas_used`를 `benchmarks/privacy-localnet-tps` report에 포함합니다. Tx별 `success`가 누락된 입력은 failed tx로 계산합니다. Public Chain TPS claim용 account pool, batch/open-loop submission, reserve/config/saturation evidence 수집은 [Clairveil Public Capacity Claim 실행 계획](clairveil-public-capacity-claim-execution-plan-kr.md)의 Track B에서 관리합니다.

### Phase B4.5: user latency trace benchmark 추가

- CLI privacy tx command는 `CLAIRVEIL_PRIVACY_LATENCY_TRACE_FILE`이 설정된 경우에만 wallet flow phase trace를 JSONL로 기록합니다.
- `cmd/clairveil-userlatency`는 trace를 flow 단위로 집계해 `prepare_latency_ms`, `proof_latency_ms`, `time_to_submit_ms`, `submit_ready_ms`, `total_latency_ms`, `error_rate`, `timeout_rate`, `cancel_rate`, optional inclusion latency를 structured summary로 출력합니다.
- `make privacy-user-latency-bench`는 localnet e2e smoke를 trace enabled로 실행하고 `benchmarks/privacy-user-latency` report를 생성합니다.

현재 상태: native 하네스 구현 완료. Native CLI smoke/reference user latency는 별도 result family로 생성할 수 있습니다. Browser/WASM adapter가 없는 동안 browser user latency public claim은 `browser_adapter_ready=true` evidence를 만족할 수 없으므로 승격 대상이 아닙니다. Remote/browser public latency claim용 trace 수집, browser adapter evidence, eligible prover RPS report 연결은 [Clairveil Public Capacity Claim 실행 계획](clairveil-public-capacity-claim-execution-plan-kr.md)의 Track C에서 관리합니다.
...
- Chain TPS public claim은 before/after reserve snapshot file/SHA-256, throughput window, saturation profile evidence를 포함합니다.
- public claim structured row는 `claim_type`과 claim별 bucket metadata를 포함하고, metric SLO는 해당 claim row 안의 metric만으로 평가됩니다.
- Prover RPS는 concurrency sweep, Chain TPS는 target tx/sec sweep이 없으면 공개 capacity claim으로 승격하지 않습니다.
