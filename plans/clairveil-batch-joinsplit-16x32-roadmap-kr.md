...
| 항목 | 내용 |
| --- | --- |
| 상태 | **BLOCKED** (2026-07-12 독립 재검증에서 Gate 3B FAIL; `PUBLICATION_READY_EXPERIMENTAL` 철회) |
| 작성일 | 2026-07-10 |
...
| --- | --- | --- | --- | --- | --- |
| 1. 현재 보안 수정 | Complete historical Gate 1; S4-B03 closed, S4-B02 re-entry active | `e427370` | historical `14d85f5`; S4-B03 regression `02f61f3746b67d5244c160b7c0e0e42f7c0b78b8` | 기존 unit/proto/전체 Go/examples/vulncheck/localnet/privacy E2E/payroll live/release-check/bulk readiness/fuzz/release-pack 검증에 더해 exact 2x2/16x32 duplicate inflation circuit regression, types wire 조기 거부, keeper proof 전 거부·무상태변경, 전체 privacy unit regression 통과 | `S4-B03`은 해결함. Session 1/current 2x2 범위의 미해결 Critical/High는 없지만 2x2 blinding exact-reuse circuit constraint `S4-B02` security-relevant Medium 1건이 active이므로 Gate 1 증거를 publication에 재사용할 수 없음. Dependency/formal setup/audit risk도 유지 |
| 2. 기반/설계 확정 | Complete historical Gate 2; 2x2 constraint re-entry 필요 | `ad99ef7193fdc0683e483e4440e5cda1f0945432` | `f117b4f8487c78b6531efe2be1ecccccefe6c5c1` | Gate 1 재검증, NoteV1/fixed payload/exact batch effect/registry/scan/path/artifact/admission 구현, corrected 16x32 full-shape와 max wire/state gate, 전체 privacy/release/E2E 검증 통과. 후속 review-fix에서 slash denom REST binding, reserve/registry genesis linkage, historical path online admission/cancellation·cached-root fail-closed, Note memo validation/fallible encoding을 보강하고 fresh consecutive clean review 2회를 통과함 | 당시 record도 current 2x2가 blinding equality를 circuit에서 금지하지 않는 future-hardening residual을 남겼음. 2026-07-12 재검증에서 security-relevant Medium/re-entry blocker로 상향함. Historical path/RSS/process isolation과 formal setup/audit risk도 유지 |
| 3A. Circuit/chain core | Complete (Gate 3A historical completion; 2x2 R1CS/VK hardening re-entry 필요) | `b7a97acd03c5e97b9e7e0bf52197ba421feda3c8` | core `fc391f5e1d69634e0b64a14735d0956302038032`; closure `cd9b4124ee0a7d3f7faeec1e76f765ec3330a88d` | production 16x32 circuit/12-input witness, proto/types, direct/governance/authz signed raw 128 KiB cap·singleton·depth guard, keeper precharge/proof-gated atomic state, exact audit identity, global uniqueness/sequence, typed scan/minimal event, genesis/root, four-circuit artifact identity/readiness를 구현함. Direct real-proof core integration, 2x2+Batch/Batch+Batch 양방향 rollback, 59-case negative matrix, targeted composition/identity suite와 full Go/build/examples/localnet/privacy E2E/bulk/release/artifact gate를 최신 hardening 뒤 재통과함 | 2026-07-12에 current 2x2 blinding exact-reuse constraint Medium을 확인함. Frozen contract enforcement와 R1CS/VK/artifact identity 재생성이 필요해 Session 2/3A 재진입 전 publication 불가. Formal setup/audit/production artifact도 미수행 |
...
| 4. 독립 검증/공개 gate | **BLOCKED — Not publication-ready** | `d45f0753c16571743f630599776c9cd498d1e8c9` 재검증 시작 | BLOCKED record `773e97d5dac68c485479cfe8de40c1d002cb5240`; S4-B03 supplement `02f61f3746b67d5244c160b7c0e0e42f7c0b78b8` | `e427370..d45f0753c16571743f630599776c9cd498d1e8c9`에서 Gate 3B를 먼저 재구성해 FAIL로 판정했으므로 Pass A~I, fresh benchmark/localnet/full release gate를 시작하지 않음. 이후 exact duplicate inflation regression으로 `S4-B03`만 닫았으며 Gate 3B를 대체하지 않음 | Gate 3B High 2건·security-relevant Medium 2건에 더해 `S4-B01` live failover evidence와 `S4-B02` 2x2 circuit blinding reuse가 unresolved. 합계 Critical 0/High 2/security Medium 4이며 `PUBLICATION_READY_EXPERIMENTAL` 선언 불가 |
...
- `S4-B02`는 active이며 Gate 1, Gate 4, publication은 완료하지 않음. 다음 순차 단계는 Session 2 re-entry이고 Session 3A는 그 뒤에 진행함.
