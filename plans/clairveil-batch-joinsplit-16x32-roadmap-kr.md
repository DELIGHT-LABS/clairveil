...
| 3B. Client/product integration | Complete (Gate 3B 충족) | `cd9b4124ee0a7d3f7faeec1e76f765ec3330a88d` | `43b49460cacc91e27ab0ae8cdb9607b44d2edce8` + closure commit | 1..16/1..32 batch SDK, structured signing, version/hash-bound local+single-remote prover, bounded admission, idempotent broadcast, lossless typed scanner/disclosure, one-operation durable payroll, CLI/tutorial/fixture/release handoff를 구현함. 1/1·3/4·31+change·exact32·padding·restart/retry actual batch localnet과 기존 Deposit/Spend/2x2·2-message·payroll live, full Go/race/vet/examples/release-check/release-pack을 통과함 | Active Critical/High/Medium/actionable finding 0건. Formal setup/production artifact/external audit/managed production infra/downstream 제품은 미수행. In-process prove 격리와 기존 no-fixed-version Go 3건/npm low 1건을 known risk로 추적 |
| 4. 독립 검증/공개 gate | Unblocked (Not Started; Gate 3B 충족) | - | - | Session 4 독립 검증은 시작하지 않음 | fresh-session adversarial/property/fuzz 검증과 publication hygiene gate가 남음 |
