...
func main() {
	...
	flag.StringVar(&humanSummaryOut, "human-summary-out", "", "write an English all-in-one human benchmark summary Markdown report to this path")
	...
	if strings.TrimSpace(humanSummaryOut) != "" {
		...
		markdown := renderHumanSummaryMarkdown(components, time.Now().UTC().Format(time.RFC3339))
...
func humanSummaryFamilyNote(family string) string {
	...
	case "privacy-localnet-tps":
		return "Localnet smoke converted to the chain TPS schema"
	case "privacy-user-latency":
...
func renderHumanSummaryMarkdown(components []humanSummaryComponent, generatedAt string) string {
	...
	fmt.Fprintf(&b, "# Clairveil Privacy Benchmark Summary Report\n\n")
	fmt.Fprintf(&b, "> This file is generated by `clairveil-benchreport -human-summary-out`. Do not edit it manually; regenerate the benchmark artifacts and then regenerate this report.\n\n")
	...
	fmt.Fprintf(&b, "This is a development benchmark report. The `public-capacity` gate must pass before these results can support published public-capacity claims, especially for TPS, production prover throughput, or user-perceived latency.\n\n")

	renderHumanSummaryConclusion(&b, components)
	renderHumanSummaryExecution(&b, components)
	renderHumanSummaryReportStatus(&b, components)
	component, ok := findComponentByFamily(components, "privacy-circuits")
	renderHumanSummaryNativeCircuits(&b, component, ok)
	component, ok = findComponentByFamily(components, "privacy-proverd")
	renderHumanSummaryHTTPTransport(&b, component, ok)
	component, ok = findComponentByFamily(components, "privacy-proverd-load")
	renderHumanSummaryExternalProverd(&b, component, ok)
	component, ok = findComponentByFamily(components, "privacy-proverd-scale")
	renderHumanSummaryExternalProverd(&b, component, ok)
	component, ok = findPreferredFeeComponent(components)
	renderHumanSummaryLocalnet(&b, component, ok)
	component, ok = findComponentByFamily(components, "privacy-localnet-tps")
	renderHumanSummaryTPS(&b, component, ok)
	component, ok = findComponentByFamily(components, "privacy-user-latency")
	renderHumanSummaryUserLatency(&b, component, ok)
	component, ok = findComponentByFamily(components, "privacy-bulk-transfer")
	renderHumanSummaryBulkTransfer(&b, component, ok)
	component, ok = findComponentByFamily(components, "public-capacity")
	renderHumanSummaryPublicCapacity(&b, component, ok)
	renderHumanSummaryArtifacts(&b, components)
	renderHumanSummaryLimitations(&b)

	return b.String()
}

func renderHumanSummaryConclusion(b *strings.Builder, components []humanSummaryComponent) {
...
func humanSummaryHeadlineLines(components []humanSummaryComponent) []string {
	...
	if component, ok := findComponentByFamily(components, "privacy-circuits"); ok {
		if bench, ok := findBenchmark(component.Report.Benchmarks, "BenchmarkJoinSplitCircuitProve"); ok {
			lines = append(lines, fmt.Sprintf("Native JoinSplit/transfer proof averaged %s with p95 %s.", formatDurationNS(bench.NSOpMean), formatDurationNS(bench.NSOpP95)))
		}
	}
	if component, ok := findComponentByFamily(components, "privacy-proverd-load"); ok {
		...
		if best != nil {
			if latency, ok := best.Metrics["latency_ms"]; ok {
				lines = append(lines, fmt.Sprintf("External `clairveil-proverd` %s measured %.3f req/s with latency p95 %.3f ms.", best.Name, best.Metrics["requests/sec"].Mean, latency.P95))
			}
		}
	}
	if component, ok := findComponentByFamily(components, "privacy-proverd-scale"); ok {
		...
		if best != nil {
			...
			lines = append(lines, fmt.Sprintf("Proverd pool scale smoke configured %.0f endpoint(s): %.0f healthy and %.0f unhealthy; %s measured %.3f req/s.", configuredEndpointCount, metricMean(best, "endpoint_count"), metricMean(best, "unhealthy_endpoint_count"), best.Name, metricMean(best, "requests/sec")))
		}
	}
	if component, ok := findComponentByFamily(components, "privacy-localnet-tps"); ok {
		if bench, ok := firstBenchmarkByKind(component.Report.Benchmarks, "chain_tps"); ok {
			if metric, ok := bench.Metrics["tx/sec"]; ok {
				lines = append(lines, fmt.Sprintf("Localnet smoke observed %.6g tx/s, but it is scripted e2e smoke and must not be interpreted as capacity TPS.", metric.Mean))
			}
		}
	}
	if component, ok := findComponentByFamily(components, "privacy-user-latency"); ok {
		...
		if transfer != nil && deposit != nil {
			lines = append(lines, fmt.Sprintf("User-latency smoke observed deposit mean %.3f ms and all-private transfer mean %.3f ms.", metricMean(deposit, "total_latency_ms"), metricMean(transfer, "total_latency_ms")))
		}
	}
	if component, ok := findComponentByFamily(components, "privacy-bulk-transfer"); ok {
		if bench, ok := firstBenchmarkByKind(component.Report.Benchmarks, "bulk_payroll_simulation"); ok {
			lines = append(lines, fmt.Sprintf("Bulk transfer simulation estimates %.0f recipient payment(s) in about %.3f seconds at %.3f item/s.", metricMean(&bench, "recipient_count"), metricMean(&bench, "estimated_total_seconds"), metricMean(&bench, "payroll_item_per_sec")))
		}
	}
	if component, ok := findComponentByFamily(components, "public-capacity"); ok {
		lines = append(lines, fmt.Sprintf("Public-capacity aggregate reports `eligible=%t`.", component.Report.ClaimProfile.Eligible))
	}
	if len(lines) == 0 {
		lines = append(lines, "The input benchmark reports are empty, so there are no key metrics to summarize.")
	}
	return lines
}

func renderHumanSummaryExecution(b *strings.Builder, components []humanSummaryComponent) {
	fmt.Fprintf(b, "## Executed Targets\n\n")
	fmt.Fprintf(b, "| Target | Result | Generated family | Notes |\n")
	fmt.Fprintf(b, "| --- | --- | --- | --- |\n")
...
func renderHumanSummaryReportStatus(b *strings.Builder, components []humanSummaryComponent) {
	fmt.Fprintf(b, "## Report Status\n\n")
...
func renderHumanSummaryNativeCircuits(b *strings.Builder, component humanSummaryComponent, ok bool) {
	if !ok {
		return
	}
	...
	fmt.Fprintf(b, "\nInterpretation:\n\n")
	fmt.Fprintf(b, "- JoinSplit proving is the largest single CPU cost.\n")
	fmt.Fprintf(b, "- Verification is measured as very small for all three circuits.\n")
	fmt.Fprintf(b, "- This table is a native microbenchmark and must not be directly converted to chain TPS or production prover throughput.\n\n")
}

func renderHumanSummaryHTTPTransport(b *strings.Builder, component humanSummaryComponent, ok bool) {
	if !ok {
		return
	}
	...
	fmt.Fprintf(b, "\nInterpretation:\n\n")
	fmt.Fprintf(b, "- These results measure HTTP client/server adapter and JSON payload-processing overhead.\n")
	fmt.Fprintf(b, "- They do not include actual Groth16 proving.\n\n")
}

func renderHumanSummaryExternalProverd(b *strings.Builder, component humanSummaryComponent, ok bool) {
	if !ok {
		return
	}
	...
	fmt.Fprintf(b, "\nInterpretation:\n\n")
	fmt.Fprintf(b, "- These results come from HTTP requests to an actual `clairveil-proverd` process.\n")
	fmt.Fprintf(b, "- Because the smoke duration is short, production-capacity claims require a longer steady state, saturation sweep, and machine/configuration evidence.\n")
	if isScale {
		fmt.Fprintf(b, "- `unhealthy_endpoint_count` must be zero for a public claim.\n")
	}
	fmt.Fprintf(b, "\n")
}
...
func renderHumanSummaryLocalnet(b *strings.Builder, component humanSummaryComponent, ok bool) {
	if !ok {
		return
	}
	rep := component.Report
	...
	fmt.Fprintf(b, "## Localnet Fees and Reserve\n\n")
	...
	fmt.Fprintf(b, "\nInterpretation:\n\n")
	fmt.Fprintf(b, "- Fees are estimates based on observed `gas_used` and exclude prover infrastructure costs.\n")
	fmt.Fprintf(b, "- The reserve snapshot checks the accounting invariant for the localnet smoke flow.\n\n")
}

func renderHumanSummaryTPS(b *strings.Builder, component humanSummaryComponent, ok bool) {
	if !ok {
		return
	}
	...
	fmt.Fprintf(b, "\nInterpretation:\n\n")
	fmt.Fprintf(b, "- This is not capacity TPS; it converts a localnet scripted e2e smoke run to the TPS schema.\n")
	fmt.Fprintf(b, "- A public TPS claim requires a target tx/sec sweep and positive inclusion-latency measurements.\n\n")
}

func renderHumanSummaryUserLatency(b *strings.Builder, component humanSummaryComponent, ok bool) {
	if !ok {
		return
	}
	...
	fmt.Fprintf(b, "\nInterpretation:\n\n")
	fmt.Fprintf(b, "- These values break down user-perceived latency by flow in a warm native smoke run.\n")
	fmt.Fprintf(b, "- Small sample counts cannot support p95/p99 public claims.\n\n")
}

func renderHumanSummaryBulkTransfer(b *strings.Builder, component humanSummaryComponent, ok bool) {
	if !ok {
		return
	}
	...
	fmt.Fprintf(b, "\nInterpretation:\n\n")
	fmt.Fprintf(b, "- This is a synthetic planning estimate based on the current one-output transfer model.\n")
	fmt.Fprintf(b, "- Production-capacity claims additionally require measurements for prover-pool saturation, localnet/chain transaction inclusion, and retry/reconciliation overhead.\n\n")
}

func renderHumanSummaryPublicCapacity(b *strings.Builder, component humanSummaryComponent, ok bool) {
	if !ok {
		return
	}
	rep := component.Report
	fmt.Fprintf(b, "## Public Capacity Decision\n\n")
	fmt.Fprintf(b, "The final decision in `public-capacity/latest.json` is `claim_eligible=%t`.\n\n", rep.ClaimProfile.Eligible)
	if len(rep.ClaimProfile.BlockingReasons) > 0 {
		fmt.Fprintf(b, "Primary blockers:\n\n")
	...
	fmt.Fprintf(b, "- An aggregate failure does not mean the benchmark execution failed.\n")
	fmt.Fprintf(b, "- It means the current artifacts lack enough evidence and samples to be promoted to published metrics.\n\n")
}

func renderHumanSummaryArtifacts(b *strings.Builder, components []humanSummaryComponent) {
...
func renderHumanSummaryLimitations(b *strings.Builder) {
	fmt.Fprintf(b, "## Current Limitations\n\n")
	fmt.Fprintf(b, "1. This document is generated automatically but assumes that the input `latest.json` files for each family are fresh.\n")
	fmt.Fprintf(b, "2. A public claim requires a long-running `public_claim` profile, not a smoke run.\n")
	fmt.Fprintf(b, "3. User latency and chain TPS cannot support published p95/p99 claims when samples or evidence are insufficient.\n")
	fmt.Fprintf(b, "4. If external proverd load measured only a specific profile, capacity for other profiles must be measured separately.\n")
}
