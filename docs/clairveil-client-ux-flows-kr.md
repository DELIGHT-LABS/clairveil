...
Chain core가 `MsgBatchTransfer`를 지원하지만 현재 배포된 product UX는 여전히 deposit, native 2x2 transfer, withdraw입니다. Repository는 reference builder, prover route, decrypting scanner, reconciliation path를 제공하지만 downstream product가 이 contract를 end-to-end로 통합하고 검증할 때까지 batch UX를 unavailable로 표시합니다. 현재 `transfer-batch` command는 여러 기존 2x2 operation을 coordination할 뿐 하나의 16x32 proof처럼 표현하면 안 됩니다.
