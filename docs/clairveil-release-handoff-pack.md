...
| Session 3B client integration pack | `docs/clairveil-session3b-batch-transfer-handoff.md`, `docs/clairveil-batch-joinsplit-localnet-tutorial.md`, `x/privacy/client/sdk/conformance/testdata/privacy_batch_transfer_session3b_contract.json` | Go/JS SDK, wallet, payroll, prover, operations teams | One-proof prepare/prove/broadcast/typed-scan/reconcile contract, boundary cases, and runnable localnet handoff |
...
`make release-pack-verify` verifies the handoff pack's external `.sha256`, canonical complete internal `SHA256SUMS.txt`, 125 required handoff files, exact selected Git file set, canonical manifest, safe canonical tar members, and every non-generated Git blob and exact Git-derived `0644`/`0755` permission mode. When `RELEASE_PACK_ARCHIVE` is not set, it regenerates the default pack before validation so stale local archives do not mask missing files. For an externally supplied archive, set `RELEASE_PACK_ARCHIVE`, `RELEASE_PACK_CHECKSUM`, and the out-of-band lowercase 40-character SHA in `RELEASE_PACK_EXPECTED_COMMIT`; the exact commit must exist in the local clone. This step checks that the tarball is not just created, but suitable to hand off as a complete release contract bundle.
...
13. Session 3A does not provide a public `MsgBatchTransfer` Go SDK, wallet scanner/decrypt UX, one-proof payroll workflow, or batch CLI/tutorial. Plan those as explicit Session 3B integrations rather than adapting the existing multi-message `transfer-batch` flow implicitly.
...
| Session 3A batch boundary | Circuit, `MsgBatchTransfer`, keeper, deterministic gas, typed scan/minimal event, genesis, and development artifact readiness are implemented; public SDK/prover route/wallet/payroll/CLI, formal setup, and production artifact delivery are not | Integrate the chain core only from the frozen proto/identity; schedule user-facing surfaces as Session 3B and artifact release as a separate security gate. |
...
8. All teams accepted the fresh-genesis `privacy-note-v1` / `privacy-fixed-v1` compatibility break, verified both independent Session 2 fixtures, and recorded `batch-joinsplit-16x32-v1` as the fourth required production chain-core circuit.
9. All teams recorded that Session 3B user-facing batch surfaces, formal trusted setup, and production artifact distribution remain incomplete.
