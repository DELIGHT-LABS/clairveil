...
12. The vendored ClairveilJS 0.2.0 package implements the `privacy-fixed-v1` fixtures and V5/V2 transfer/withdraw contracts. Treat pre-0.2 note cache, reservation state, and relay recovery metadata as a fresh-genesis boundary: discard or namespace it and rescan; never add a compatibility decoder.
...
| External ClairveilJS compatibility | The vendored 0.2.0 package implements `privacy-fixed-v1` and the V5/V2 prepared-payload/proof contracts; pre-0.2 browser persistence is incompatible | Start a fresh cache/reservation/relay namespace, rescan, and never add a compatibility decoder. Prepared transfer payload `v5` remains a separate outer version. |
...
9. All teams recorded that the user-facing Go batch-reference surfaces are implemented, while downstream JS/web product delivery, formal trusted setup, and production artifact distribution remain incomplete.
