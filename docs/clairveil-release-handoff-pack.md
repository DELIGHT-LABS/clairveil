...
13. Session 3B now provides the public Go `MsgBatchTransfer` SDK, wallet scanner/decrypt path, one-proof payroll workflow, and batch CLI/tutorial that Session 3A intentionally omitted. Port those frozen reference contracts explicitly into downstream JS/TS and web products; do not treat the legacy multi-message `transfer-batch` flow as an implicit compatibility layer or confuse the Go reference with completed production deployment.
...
| Session 3A/3B batch boundary | Circuit, `MsgBatchTransfer`, keeper, deterministic gas, typed scan/minimal event, genesis, and the Session 3B Go SDK/prover/scanner/payroll/CLI reference surfaces are implemented; external JS/web product delivery, formal setup, and production artifact delivery are not | Integrate against the frozen proto/identity and Go reference contracts, while tracking downstream product work and production artifact release as separate production gates. |
...
9. All teams recorded that the Session 3B Go reference user-facing batch surfaces are implemented, while downstream JS/web product delivery, formal trusted setup, and production artifact distribution remain incomplete.
