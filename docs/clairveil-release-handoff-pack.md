...
13. The batch integration provides the public Go `MsgBatchTransfer` SDK, wallet scanner/decrypt path, one-proof payroll workflow, batch CLI/tutorial, and the ClairveilJS 0.3.1 batch API. The checked-in WebApp intentionally keeps `serverFeatures.batchTransfer=false` and exposes no one-proof batch submission or authorized batch-audit UI. A downstream product must add its own reviewed checkpoint, wallet confirmation, reconciliation, and E2E release gates before enabling that SDK surface; do not treat SDK availability as completed product deployment.
...
| Batch chain-core/client-integration boundary | Circuit, `MsgBatchTransfer`, keeper, deterministic gas, typed scan/minimal event, genesis, reference Go SDK/prover/scanner/payroll/CLI, and ClairveilJS 0.3.1 batch APIs are implemented; the checked-in WebApp batch product flow, formal setup, and production artifact delivery are not | Keep the example batch gate false. Treat a downstream batch WebApp, checkpoint/recovery design, product E2E, and production artifact release as separate gates. |
...
9. All teams recorded that the Go reference and ClairveilJS 0.3.1 batch surfaces are implemented, while the checked-in WebApp batch product flow, formal trusted setup, and production artifact distribution remain incomplete.
