...
Restoring zero deposits removes the non-zero constraint from `DepositAuditFieldV1`, making existing deposit R1CS/PK/VK artifacts incompatible. Run `clairveil-setup --out <new-directory> --development` from the current source to regenerate the full four-circuit bundle and manifest, then bind the new manifest’s `CircuitSetIdentity` in the fresh-genesis audit configuration. An unchanged circuit-set name does not permit reuse of old keys or manifests; node/prover strict identity checks must pass. This remains a development setup, not a production trusted setup.
