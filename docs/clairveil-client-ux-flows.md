...
The browser example keeps native 2x2 transfer as its default entry point and exposes one-proof `MsgBatchTransfer` only behind the Cosmos `batchTransfer` feature gate. The batch screen calls `prepareTransferBatch`, shows payment total, change, input/output capacity, and the all-or-nothing boundary, then reports each item successful only after typed output evidence reconciles. The older `transfer-batch` command merely coordinates multiple existing 2x2 operations and must not be presented as one 16x32 proof.
