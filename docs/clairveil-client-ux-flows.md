...
## 10. Batch Chain Core And Client UX

The browser example keeps native 2x2 transfer as its single-transfer entry point and exposes one-proof batch transfer behind the `batchTransfer` feature gate when the selected transport supports it. The batch screen calls `prepareTransferBatch`, shows payment total, change, input/output capacity, and the all-or-nothing boundary, then submits one Cosmos `MsgBatchTransfer` or one EVM `singleProofBatchTransfer` call. It reports each item successful only after typed output evidence reconciles.

Initialize the client from fresh genesis with exact `privacy-note-v1` artifacts and empty note, cursor, reservation, and proof namespaces. Accept only canonical `privacy-fixed-v1` typed note/disclosure envelopes and verified proof objects. Resolve display denoms through authoritative `AssetRegistryV1`; an unknown `asset_id` is an error rather than a display denomination.

Store the unified scan cursor as `(height, global_sequence, output_index)` and only mark it committed after all outputs through that cursor are durable. A spend screen must use a Merkle path snapshot for exactly the root shown to the prover. Current-root paths use incremental nodes and do not consume the online historical-rebuild budget. A non-current historical path requires persisted root/count/height metadata; the public query admits at most 1,024 leaves and two concurrent rebuilds per keeper, otherwise it returns `ResourceExhausted`. Use the current root or a trusted local historical index above that online bound. The separate offline recovery/export bound remains `MaxMerkleRebuildLeaves` (1,048,576). If the wallet performs a remote historical lookup, retain the warning that the request can reveal when and which state the wallet is interested in.

The one-proof batch review screen uses the production counts and roots. Its 12 public inputs are ordered as `MerkleRoot`, `ChainDomainHi`, `ChainDomainLo`, `ExpiresAtUnix`, `InputCount`, `OutputCount`, `NullifierRoot`, `CommitmentRoot`, `UserDisclosureRoot`, `FullDisclosureRoot`, `PayloadDigestHi`, `PayloadDigestLo`. A prover-busy response is retryable against the same endpoint; automatic cross-endpoint failover remains off. Canceling the screen does not prove that in-process proving stopped, so keep notes reserved until transaction/proof-job state is reconciled. Never silently split a capacity overflow: require the user to opt into multiple independent atomic batches and explain that completed batches remain committed if a later batch fails.
