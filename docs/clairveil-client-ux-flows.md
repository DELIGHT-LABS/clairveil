...
## 10. Session 3A Core And Session 3B Batch UX

The shipped product UX remains deposit, native 2x2 transfer, and withdraw even though the chain core now supports `MsgBatchTransfer`. Label the batch UX unavailable until Session 3B supplies its builder, prover route, decrypting scanner, and reconciliation path. The current `transfer-batch` command merely coordinates multiple existing 2x2 operations and must not be presented as one 16x32 proof.
...
The Session 3B batch review screen may be designed around the production counts and roots, but it must not imply that the client path is ready. Its 12 public inputs are ordered as `MerkleRoot`, `ChainDomainHi`, `ChainDomainLo`, `ExpiresAtUnix`, `InputCount`, `OutputCount`, `NullifierRoot`, `CommitmentRoot`, `UserDisclosureRoot`, `FullDisclosureRoot`, `PayloadDigestHi`, `PayloadDigestLo`. A prover-busy response is retryable against the same endpoint; automatic cross-endpoint failover remains off. Canceling the screen does not prove that in-process proving stopped, so keep notes reserved until transaction/proof-job state is reconciled.
