# Batch JoinSplit 16x32 독립 공개 검증 보고서

## 2026-07-13 독립 공개 검증 완료 — 현재 authoritative record

| 항목 | 결과 |
| --- | --- |
| 보안 강화 기준 commit | `e42737022b2aa87b498f57ae4d089ccb84a45968` |
| 공개 검증 시작 snapshot | `0df27417910f46ff714e73ce0730f5e167ece33a` |
| 검증된 immutable 구현 snapshot | `a84ca1cc1cd835990243d9b3f5f064e7b538f7ae` |
| 최종 검토 범위 | `e42737022b2aa87b498f57ae4d089ccb84a45968..HEAD`; 시작 snapshot은 변경하지 않음 |
| 검증 상태 | Security, protocol, chain-core, client-integration gates **PASS**, 독립 공개 검증 Pass A~I **PASS** |
| Finding 상태 | unresolved Critical `0`, High `0`, security-relevant Medium `0` |
| Publication | **`PUBLICATION_READY_EXPERIMENTAL`** |
| Production | **`PRODUCTION_RELEASE_READY` 승인 안 함** |

이 record가 현재 authoritative 상태다. 아래 2026-07-12/이전 publication-ready, client-integration gate FAIL/PASS, 독립 공개 검증 `BLOCKED` 문단은 provenance를 보존한 historical record이며 현재 판정이나 evidence로 사용하지 않는다. 검토는 초기 range를 read-only로 유지한 채 reviewer 2명의 단일 병렬 wave(Pass A~E, Pass F~I/publication hygiene)로 수행했고, main reviewer가 모든 후보를 직접 코드와 테스트로 판정했다. 반복 reviewer wave나 clean-round loop는 수행하지 않았다.

### Finding closure

| ID | Severity | 독립 판정과 수정 | 상태 |
| --- | --- | --- | --- |
| `PROVER-FAILOVER-LIVE-EVIDENCE` | Medium, security-relevant | 실제 timeout/healthy HTTP prover 두 endpoint의 server-side request counter와 수신 body를 관찰한다. Default no-failover는 접촉 `1/0`, explicit opt-in은 `1/1`이며 healthy 응답으로 완료된다. Timeout, malformed response, validation failure를 별도 회귀하고 body/witness/nullifier/path/amount는 출력하지 않는다. Commit `1b1dc08` | **RESOLVED** |
| `NONCANONICAL-FIELD-ALIAS` | Medium, security-relevant | 동일 BN254 residue를 가진 `r`과 `r+q`가 commitment는 같지만 raw decimal secret 비교를 우회했다. Canonical field byte 비교와 non-canonical alias fail-before-signature regression을 추가했다. Commit `cdc7780` | **RESOLVED** |
| `LOCALNET-PROCESS-CLEANUP` | Low, operational | Reference payroll localnet이 background shell function PID를 정리해 node를 orphan으로 남겼다. Node process를 직접 background하고 rerun 뒤 node/prover 잔존 0을 확인했다. Commit `a84ca1c` | **RESOLVED** |
| `PUBLICATION-DOC-CONSISTENCY` | Medium, publication hygiene | 한영 normative/handoff 문서의 stale client-integration gate FAIL, `PROVER-FAILOVER-LIVE-EVIDENCE` active, 독립 공개 검증 `BLOCKED` current-state 문구를 historical record와 구분해 정렬했다. | **RESOLVED** |

### Pass A~I fresh 결과

| Pass | 독립 검증 | 결과 |
| --- | --- | --- |
| A | current authorization/intent, disclosure dictionary, commitment collision, crypto decoder, privacy default, genesis/artifact identity | **PASS** |
| B | NoteV1 native/Deposit/Spend/2x2/Batch/scanner/genesis 및 independent KAT 일치 | **PASS** |
| C | Batch 12 public input 순서와 circuit/native/keeper/SDK/prover witness 독립 재구성 | **PASS** |
| D | active prefix, disabled sentinel, duplicate, value/root/path/owner/signature/disclosure adversarial witness | **PASS** |
| E | input `1..16`, output `1..32`, differential/property, independent vector/payload KAT와 bounded fuzz | **PASS** |
| F | keeper framing/gas/precharge, consensus identity, atomic rollback, cross-message composition, resource bounds | **PASS** |
| G | minimal event, one-copy typed scan plane, sequence/cursor/restart/genesis/item evidence | **PASS** |
| H | role-aware artifact admission, body/queue/permit bound, payload-free error, live no-failover, safe view tag | **PASS** |
| I | one-proof payroll graph/worker/broadcast/reconcile와 batch/item evidence 분리 | **PASS** |

모든 targeted 명령은 실제 `--- PASS` test name을 확인했다. `SKIP` 또는 `[no tests to run]`은 PASS로 처리하지 않았다. PostgreSQL이 없는 첫 실행을 skip으로 수용하지 않고 Docker PostgreSQL 17을 준비해 actual integration을 완료했다.

### `PROVER-FAILOVER-LIVE-EVIDENCE` live evidence

| 시나리오 | timeout 접촉 | healthy 접촉 | 관찰 |
| --- | ---: | ---: | --- |
| Default no-failover | 1 | 0 | timeout body만 server-side 관찰, healthy body 없음 |
| Explicit opt-in failover | 1 | 1 | 두 endpoint body 일치, healthy 응답으로 정상 완료 |

Timeout, malformed response, response validation failure는 각각 다른 error class이며 각 endpoint 접촉 횟수는 1이다. `automatic_multi_prover_failover=false`는 JSON literal이 아니라 이 실제 counter 결과에서 생성된다. Evidence에는 count/boolean만 기록하고 private request body는 기록하지 않는다.

### Benchmark와 resource evidence

환경은 Apple M5 Pro/64 GiB, Darwin arm64, Go `1.25.12`, gnark `v0.14.0`, Groth16/BN254다. `TestBatchJoinSplit16x32FullShapeResourceGate`는 61.62초에 PASS했고 process peak RSS는 `3,354,689,536 B`다.
...
### Integration, fuzz와 release evidence

- `make reservation-sql-integration`: actual SQLite와 Docker PostgreSQL 17 graph atomicity/recovery exact test가 각각 PASS, skip 0.
- `make validate-joinsplit-artifact-rotation-evidence`: current/previous artifact rotation/tamper, old/new proof identity, fresh genesis identity를 source regeneration으로 PASS.
- 10개 actual fuzz target을 각각 `-fuzztime=3s -parallel=4`로 실행해 PASS: note/disclosure plaintext, batch/2x2 payload, vector root, prover JSON, scan page, EC point, EdDSA signature, ECIES envelope.
- `make privacy-e2e-smoke`: Deposit/disclosure/2x2/Withdraw PASS.
- `RUN_LOCALNET=1 make privacy-batch-joinsplit-localnet`: 5 shape, one proof/one tx envelope payroll, node/prover restart, timeout/exact retry, tx-hash-first reconcile, spent-nullifier rejection, disclosure/view-tag safe scan, genesis export/import와 post-import continuation PASS. Gas는 `1/1` `1,609,514`, `3/4` `3,141,099`, `31+change` `16,017,355`, exact `32` `16,876,619`, padding `15,529,326`이다.
- `make reference-payroll-live-localnet`: chunk 1개/2 item, confirmed item 2, spent reservation 4, 최종 `Confirmed`, 종료 후 node/prover process 0으로 PASS.
- `go test ./... -count=1`, `go test -race ./x/privacy/... -count=1`, `go vet ./...`, `make ci`, `make vulncheck`, `make examples`, `make release-check`, `git diff --check`: PASS.
- Tracked secret, generated R1CS/PK/VK/archive, `dist/`/`benchmarks`/`tmp`/개인 absolute path는 0이다. Public input/NoteV1/payload/schema/version/circuit statement는 변경하지 않았다.
- Final clean immutable commit에서 `make release-pack`과 `make release-pack-verify`를 실행하고 동일 archive를 Python 3.9 및 3.12에서 검증한다. Required file은 125개이며 exact source commit은 `RELEASE-MANIFEST.txt`, archive SHA-256은 co-located `.sha256`에 기록한다. Archive 내부에 self-referential checksum/commit placeholder를 두지 않는다.

### Accepted residual과 Production TODO

| Residual/TODO | Owner | 수용 이유 | Production blocking |
| --- | --- | --- | --- |
| External ZK audit, final source/constraint freeze, official MPC/trusted setup, transcript/toxic-waste evidence | Protocol/release owner | Experimental source gate의 명시적 비목표이며 development setup만 존재 | Yes |
| Signed production artifact/provenance/custody, production manifest/SBOM/image provenance | Release/validator operator | Target release infrastructure와 signing policy 필요 | Yes |
| Production gas governance/calibration, upgrade/rollback, staging load/fault, monitoring/incident response | Downstream chain owner | Target chain 운영 범위 | Yes |
| Prover TLS/auth/ACL/quota/process isolation/retention, audit-key custody/rotation/manual review | Prover 및 auditor/payroll operator | Managed production infrastructure와 절차 필요 | Yes |
| Downstream JS/TS wallet/product, metadata leakage와 padding policy | Product/privacy owner | Go reference만 publication 승인; product acceptance 별도 | Downstream production은 Yes |
| No-fixed-version Go advisory 3건과 example npm Low 1건 | Dependency/security owner | Existing exact policy로 추적하고 숨기지 않음 | Production 전 재평가 |

위 항목은 security finding을 residual로 전환한 것이 아니다. External audit/freeze/MPC/transcript/signed provenance/downstream rollout이 끝날 때까지 `PRODUCTION_RELEASE_READY`를 승인하지 않는다.

## Historical 상태와 이전 검증 기록 (Superseded)

아래의 모든 current/blocked/finding 표현은 각 기록 시점의 historical 사실이다. 현재 상태는 위 2026-07-13 독립 공개 검증 Closure만 결정한다.

| 항목 | 결과 |
| --- | --- |
| 검토 범위 | `e427370..d45f0753c16571743f630599776c9cd498d1e8c9` |
| 검증한 시작 HEAD | `d45f0753c16571743f630599776c9cd498d1e8c9` |
| 검토 역할 | 보안 강화, protocol, chain-core, client-integration 구현에 참여하지 않은 fresh reviewer |
| 현재 client-integration gate | **PASS — 네 가지 client-integration finding(`BATCH-SIGNER-SECRET-FRESHNESS`, `SQL-GRAPH-ATOMICITY`, `ONE-PROOF-PAYROLL-E2E`, `LIVE-DISCLOSURE-VERIFICATION`)의 re-entry closure 완료** |
| `DISCLOSURE-BLINDING-SEPARATION` batch chain core 보완 | **RESOLVED — security, protocol, and chain-core gates fresh closure PASS, batch reference integration re-entry UNBLOCKED** |
| 독립 공개 검증 실행 상태 | **`UNBLOCKED / NOT STARTED`** |
| 독립 공개 검증 공개 상태 | **`BLOCKED`** (`PROVER-FAILOVER-LIVE-EVIDENCE` 및 독립 Pass A~I 미수행; `PUBLICATION_READY_EXPERIMENTAL` 철회 유지) |
| production release 상태 | 승인하지 않음 |
| formal trusted setup | 수행하지 않음 |
| external audit | 수행하지 않음 |

이 2026-07-12 재검증은 같은 날 앞서 완료된 publication claim을 supersede한다. 2026-07-13 batch reference integration re-entry closure가 위 네 가지 client-integration finding을 닫아 client-integration gate를 PASS로 전환했다. 독립 공개 검증 Pass A~I는 아직 시작하지 않았고 `PROVER-FAILOVER-LIVE-EVIDENCE`이 남아 있으므로 experimental source 공개 승인은 계속 `BLOCKED`다.

### 2026-07-13 batch client 통합 재검증 완료 보충 기록

- 시작 HEAD `16d2280`; 구현 commit은 `79ea24e`(`BATCH-SIGNER-SECRET-FRESHNESS`), `bbf168f`(`SQL-GRAPH-ATOMICITY`), `1e855cb`(`ONE-PROOF-PAYROLL-E2E`), `009cc36`(`LIVE-DISCLOSURE-VERIFICATION`)이다.
...
- batch reference integration targeted/full privacy/race/vet, actual SQL, localnet, build/examples/release-check/release-pack/verify/diff를 통과했다. Public input, NoteV1, payload/schema/version, circuit contract는 변경하지 않았다.
- **client-integration gate PASS. 독립 공개 검증 UNBLOCKED / NOT STARTED.** 이 supplement는 독립 공개 검증의 독립 Pass A~I, fresh benchmark/fuzz/publication hygiene를 수행하거나 승인하지 않는다.
- 독립 공개 검증 active finding은 `PROVER-FAILOVER-LIVE-EVIDENCE` security-relevant Medium 1건이다. Current unresolved count는 Critical 0, High 0, security-relevant Medium 1이다.
...
- `ARTIFACT-BYTE-IDENTITY`는 current/previous 모든 required artifact actual SHA-256, non-JoinSplit byte identity, canonical descriptor, missing/duplicate/unknown/tamper fail-closed, positive JoinSplit-only rotation과 exact current-source R1CS serialization identity를 검증한다. `make validate-joinsplit-artifact-rotation-evidence`에 포함되며 clean commit에서는 pinned prior/current source로 두 set을 재생성할 수 있다.
...
- `DISCLOSURE-BLINDING-SEPARATION`과 `DUPLICATE-INFLATION-REGRESSION` targeted test, artifact/proof/genesis evidence, 전체 privacy/repository test, vet/build/examples, JoinSplit/Batch resource gate, release-check, diff, 한영 문서 및 secret/artifact hygiene가 PASS했다. Exact release pack은 closure commit 뒤 Python 3.9/3.12에서 검증한다.
- 당시 처분은 **security, protocol, and chain-core gates PASS, batch reference integration re-entry UNBLOCKED**였다. 당시 위 네 가지 client-integration finding과 `PROVER-FAILOVER-LIVE-EVIDENCE` 때문에 client-integration gate, 독립 공개 검증, publication은 계속 **BLOCKED**였다.

### Disclosure-Blinding Separation batch chain-core 구현 보충 기록

- 기준 HEAD `0fc818c`; 구현 commit은 `0b7d97d`, `630736f`, `25c17ef`이다. Latest Master Ledger와 batch protocol contract re-entry record가 authoritative source다.
...
- `DISCLOSURE-BLINDING-SEPARATION` 구현은 **RESOLVED**다. 후속 2026-07-13 supplement가 security, protocol, and chain-core gates fresh closure를 PASS하고 batch reference integration re-entry를 UNBLOCK했다. 당시 unresolved count는 Critical 0, High 2, security-relevant Medium 3이며 독립 공개 검증은 재개하지 않았다.
...
- Public input/NoteV1/payload encoding/disclosure digest/circuit-set version은 변경하지 않는다. batch chain core는 JoinSplit R1CS/PK/VK와 manifest/consensus JoinSplit identity만 교체한다.
- batch chain core re-entry는 **UNBLOCKED / NOT STARTED**이며 `DISCLOSURE-BLINDING-SEPARATION`은 production constraint/artifact/pre-sign enforcement와 regression/readiness/resource gate 완료 전까지 **IMPLEMENTATION PENDING / NOT RESOLVED**다.
- `DUPLICATE-INFLATION-REGRESSION`은 `02f61f3746b67d5244c160b7c0e0e42f7c0b78b8`, `42d40bd19523e263aaf1c2043bcd274a4fc1a51d`에서 **RESOLVED**다.

## Historical — 2026-07-12 재검증 Finding (네 가지 client-integration finding과 `PROVER-FAILOVER-LIVE-EVIDENCE`는 후속 closure에서 RESOLVED)

아래 네 가지 client-integration finding 행은 finding provenance를 보존한 historical entry이며 모두 위 batch reference integration closure로 해결됐다. `PROVER-FAILOVER-LIVE-EVIDENCE`만 독립 공개 검증의 active finding이다.

| ID | Severity | 근거 | 영향 범위 | 필요한 조치 |
| --- | --- | --- | --- | --- |
| ONE-PROOF-PAYROLL-E2E | High | Batch localnet은 one-proof transfer shape만 실행하고 payroll operation graph/worker/reconcile/report를 사용하지 않는다. Reference payroll localnet은 legacy multi-message 2x2 `transfer-batch` 경로다. | one-proof payroll reserve, prove, signed-byte retry, typed item evidence, reconcile/report가 실제 chain에서 연결된 적이 없다. | batch reference integration에서 production payroll worker 전체 경로의 fresh localnet E2E를 추가하고 restart/retry까지 검증한다. |
| LIVE-DISCLOSURE-VERIFICATION | High | Localnet은 mixed disclosure/self-view 옵션만 생성하고 recipient/auditor/self-view decrypt, blinding 기반 digest 재계산, expected output 전체 count/commitment, view-tag mismatch safe scan을 assert하지 않는다. | Typed scanner 또는 disclosure consumer가 output을 누락해도 runner가 성공할 수 있다. | Live disclosure consumer와 view-tag mismatch injection을 추가하고 output별 evidence를 검증한다. |
| SQL-GRAPH-ATOMICITY | Medium, security-relevant | `SQLStore` 구현은 있으나 test는 schema 문자열, placeholder, isolation option만 검사한다. 실제 SQLite/PostgreSQL CRUD/rollback/reopen/lease-CAS 실행은 없다. | SQL backend의 reservation-operation-item-evidence 원자성이 입증되지 않아 orphan/duplicate/wrong item status 위험이 남는다. | 최소 실제 SQLite transaction/restart/rollback/concurrency test를 추가한다. |
| BATCH-SIGNER-SECRET-FRESHNESS | Medium, security-relevant | `ValidateBatchTransferSigningRequest`는 final prepared validator와 달리 input/output 및 output 간 global secret reuse를 서명 전에 거부하지 않는다. | 비신뢰 preparer가 privacy-leaking intent에 owner signature를 먼저 얻을 수 있다. | Structured signer validator에 동일한 `seenSecrets` 검사를 넣고 adversarial signing test를 추가한다. |
| PROVER-FAILOVER-LIVE-EVIDENCE | Medium, security-relevant | Default no-failover와 explicit opt-in unit test는 있지만 localnet은 timeout/healthy endpoint의 실제 접촉 횟수를 측정하지 않는다. 결과 JSON 값은 실행 관찰이 아닌 literal이다. | 실제 transport에서 witness가 두 번째 prover로 전송되지 않는 privacy default를 publication evidence로 입증하지 못한다. | 두 endpoint live harness에서 default와 opt-in을 각각 검증한다. |

Resolved supplements: `DUPLICATE-INFLATION-REGRESSION`은 `02f61f3`/`42d40bd`, `DISCLOSURE-BLINDING-SEPARATION` 구현은 `0b7d97d`/`630736f`/`25c17ef`로 닫혔으며 현재 finding count에 포함하지 않는다.

## Historical — 2026-07-12 검증 처분 (Superseded)

- client-integration gate FAIL 때문에 독립 공개 검증 Pass A~I, fresh max-shape benchmark, fresh localnet, full regression/race/fuzz/release gate는 **수행하지 않았다**. 아래 historical 결과는 현재 gate evidence로 재승인하지 않는다.
...
- Payroll default no-failover/explicit opt-in, durable reconcile, prove permit lifetime, memory/file store test는 PASS했다. SQL test는 schema-only여서 `SQL-GRAPH-ATOMICITY`를 닫지 않는다.
- `/tmp/clairveil-joinsplit-artifacts-381c984`의 batch R1CS `122,813,535 B`, PK `209,218,621 B`, VK `716 B`와 SHA-256은 historical record와 일치한다.
...
`ARTIFACT-GATE-EXECUTION` 재검토에서 이전 artifact/fresh-genesis 명령의 opt-in 환경변수와 테스트명이 실제 코드와 달라 `SKIP` 또는 `[no tests to run]`을 성공으로 오인할 수 있음을 확인했다. 아래 표는 그 두 행을 폐기하고 fail-closed `make validate-joinsplit-artifact-rotation-evidence` 결과로 대체한다. 이 evidence 보정만으로 fresh security, protocol, and chain-core gates를 PASS한 것은 아니며 후속 2026-07-13 closure supplement가 세 Gate를 닫는다.

| 명령 | 결과와 한계 |
| --- | --- |
| `go test ./x/privacy/client/sdk/conformance -run '^(TestPrivacyNoteV1ContractIndependentGolden\|TestPrivacyBatchJoinSplitV1ContractIndependentGolden)$' -count=1 -v` | PASS. Independent golden 계산 경로 확인. client-integration gate 대체 아님 |
...
| `go test ./x/privacy/client/sdk/reservation -run '^(TestBatchOperationGraphIsAtomicAndConflictsWithOrdinaryReservation\|TestBatchOperationDurableFileRestartRoundTrip\|TestBatchOperationSQLSchemaIsVersionedAndRelational)$' -count=1 -v` | PASS. 실제 SQL transaction test가 아니므로 SQL-GRAPH-ATOMICITY 유지 |
...
| `make validate-joinsplit-artifact-rotation-evidence`(clean commit 재생성), 또는 supplied set용 artifact directory 변수 두 개 | PASS. `TestJoinSplitArtifactRotationSnapshotValidation`, `TestJoinSplitDevelopmentArtifactRotationGate`, `TestJoinSplitOldAndNewProofIdentitiesAreMutuallyExclusive`, `TestFreshGenesisUsesRotatedJoinSplitIdentity`를 실행해 descriptor/tamper fail-closed, actual artifact readiness, exact current-source R1CS serialization identity, old/new proof-VK 상호 거부, fresh genesis의 old identity state-write 전 거부를 확인. Wrapper가 exact 실행과 `--- PASS`를 요구하고 `SKIP`/`[no tests to run]`을 실패 처리함 |
...
| `go test ./... -count=1`; `go vet ./x/privacy/...`; `make build`; `make examples`; `make vulncheck`; `git diff --check` | PASS. batch chain core 구현 범위의 final repository/release 정적 검증이며 독립 공개 검증 Pass A~I 또는 live E2E 재개가 아님 |
...
| Pass A~I, race/fuzz, benchmark, fresh localnet/live E2E와 전체 `make release-check` | **NOT RUN — client-integration gate FAIL 및 batch chain core 범위 밖** |
...
| --- | --- | --- | --- |
| External ZK audit, source/constraint freeze, official MPC/trusted setup과 transcript | Protocol/release owner | 독립 공개 검증 비목표이며 development artifact만 존재함 | Yes |
...
이 historical 표는 당시 active client-integration 및 batch protocol/chain-core blocker를 수용하거나 publication을 승인하지 않았다.

## Historical — Prior 2026-07-12 Validation Record (Superseded)

아래 내용은 이전 reviewer가 기록한 historical claim을 provenance 목적으로 보존한다. 현재 publication 상태, Pass 결과, benchmark/localnet evidence로 사용하지 않으며 문서 앞의 2026-07-13 batch reference integration Re-entry Closure Supplement가 우선한다.

## Historical — 독립 검토 방법

Master Roadmap과 보안 강화 기준선부터 독립 공개 검증까지의 모든 plan 및 Completion Record를 처음부터 끝까지 읽고, code에서 protocol을 재구성한 뒤 normative contract와 비교했다. 초기 finding을 재현하고 판정하기 전에는 파일을 수정하지 않았다. Pass A~I보다 client-integration gate를 먼저 확인했으며 clean worktree와 integration evidence가 최초에는 충족되지 않았으므로 gate가 열린 동안 독립 공개 검증을 통과 처리하지 않았다.
...
## Historical — Finding과 수정

| ID | Severity | 근거 | 영향 범위 | 해결 |
| --- | --- | --- | --- | --- |
| BATCH-PREPARATION-REVALIDATION | High | Batch preparation과 payload builder가 mutable plan의 total, ordering, ownership, disclosure mode, field를 완전히 독립 재계산하지 않았다. | 변조된 prepared object가 planner intent와 다른 의미로 signing/prover 경계를 넘거나 sensitive work 뒤 늦게 실패할 수 있었다. | `d9b1780`: count, ownership, asset, nullifier uniqueness, output role, conservation, disclosure policy, canonical field를 재계산하고 private file을 durable atomic replacement로 기록함. |
...
| LOSSLESS-TYPED-PAGINATION | High | Typed scan page가 zero-output summary를 건너뛴 cursor advance를 증명하지 않았고 scanner가 filtered event set을 요청했다. | pagination이 정상 boundary를 거부하거나 output-bearing event 누락을 숨겨 wallet/payroll recovery가 lossless하지 않을 수 있었다. | `868f108`: unfiltered typed scan, ordered summary cursor proof, output event skip 금지, nil cursor 안정화, lossy ABCI fallback 금지를 적용함. |
| DURABLE-PAYROLL-ATOMICITY | High | Durable payroll의 reservation, operation, item, lease, broadcast, reconcile transition이 memory/file/SQL store 전체에서 일관되게 atomic/evidence-bound하지 않았다. | crash/retry race가 reservation을 고립시키거나 중복 작업을 만들고 필요한 batch-output evidence 없이 item 결과를 판정할 수 있었다. | `0b6b3ee`: atomic operation graph persistence, lease/CAS 검사, evidence-bound reconciliation, store별 regression을 추가함. |
| BATCH-LOCALNET-GATE | Medium | 초기 batch localnet rehearsal이 모든 negative 결과, artifact identity, process cleanup, repository hygiene를 강제하지 않았다. | client-integration gate evidence가 실제 integration boundary를 증명하지 않고 성공할 수 있었다. | `d7809e9`로 보강하고 targeted/full privacy/race/vet/examples/E2E/payroll/artifact/release 검증 뒤 `423f73a`에서만 client-integration gate를 재기록함. |
| CONFIRMED-FAILURE-RESIGN | High | confirmed failed payroll tx를 operation graph를 보존하면서 new-sequence 재서명 상태로 옮기는 명시적 store transition이 없었다. | 회복 가능한 chain failure가 durable dead end가 되어 수동 DB 수정 없이는 안전하게 retry할 수 없었다. | `16900cb`: `PrepareBatchOperationResign`을 lease-bound/confirmed-failure 전용으로 추가하고 nullifier를 재확인하며 explicit `ResignWithNewSequence`에서만 계속함. |
| PROPERTY-FUZZ-COVERAGE | Medium | 여러 decoder의 bounded fuzz와 모든 지원 input/output count의 cross-layer property coverage가 공개 gate 기준보다 부족했다. | canonical decoder, count, root, witness drift를 독립 검출하는 강도가 부족했다. | `2f4d065`: fuzz target 7개, circuit 16개 input shape property, SDK `1..16/1..32`, keeper 12-input property, frozen independent KAT를 추가함. |
| FIVE-SHAPE-CAPACITY-EVIDENCE | Medium | capacity evidence가 요구된 5개 shape의 distribution과 scanner/wire/state/gas profile을 모두 기록하지 않았다. | experimental 공개에서 resource envelope와 shape 비교를 근거로 제시할 수 없었다. | `7407007`: 5-shape p50/p95/max resource gate, solve/gas/wire/state/event/scanner benchmark를 추가함. |
| LOCALNET-RESTART-GENESIS-CONTINUATION | High | localnet restart helper가 shell function을 background로 실행해 기록한 PID가 node process가 아니었다. 기존 node가 살아 있으면 새 start가 DB lock으로 실패해도 health check가 통과했다. Genesis continuation도 실행하지 않았다. | restart/recovery 공개 근거가 false positive였다. | `15e644a`: process 직접 실행과 `kill -0` lifecycle 검사, 실제 node/prover restart, non-zero-height genesis export/import, cursor/cache/path/reserve/asset 비교, import 뒤 continuation을 추가함. |
| DEPOSIT-PLAINTEXT-LOGGING | High | Deposit CLI가 완전한 NotePlaintextV1 hex를 stderr에 출력했다. | receiver key, amount, randomness, memo가 terminal capture 또는 application log에 노출될 수 있었다. | `c40c865`: helper가 message만 반환하고 note plaintext를 log하지 않도록 수정함. |
| BILINGUAL-CONTRACT-ALIGNMENT | Medium | 한영 handoff/security 문서가 구현된 batch Go reference를 여전히 pending으로 표현하거나 downstream product completion과 혼동했다. | reviewer와 integrator가 code와 다른 contract 상태를 신뢰할 수 있었다. | `cee89a7`: 18개 한영 공개 문서에서 구현된 Go reference와 미완료 downstream/formal/production 작업을 분리함. |
| PROVER-BODY-HARD-LIMIT | Medium | Public low-level prover `HTTPHandler`는 batch route만 제한했고 transfer/withdraw는 admission 전에 unbounded `io.ReadAll`을 사용했다. | Handler를 직접 mount하면 공격자가 제어한 body를 permit 획득 전에 메모리에 올려 memory DoS를 만들 수 있었다. | `fc45edd`: 세 route가 같은 hard reader를 사용하고 wire/gzip overflow는 413을 반환하며 payload를 error에 포함하지 않는다. Transfer/withdraw overflow가 admission 전 거부되는 test도 추가함. |
| IMMUTABLE-VALIDATION-SNAPSHOT | Medium | 초기 Completion Record가 mutable `HEAD` range와 `독립 공개 검증 closure commit` placeholder를 사용했다. | 공개한 validation claim을 하나의 immutable source snapshot에서 재현할 수 없었다. | 이 record에서 report, Completion Record, master ledger가 검증한 implementation `494c72df2cad38dc1cc97d5e6e0f15b38e0c82d2`를 고정한다. 생성한 release manifest는 publication-record commit과 archive checksum을 별도로 정확히 기록한다. |
...
| IGNORED-FILE-EXCLUSION | Medium | Clean-worktree 검사는 ignored file을 숨기지만 recursive directory copy는 ignored `.env`, `node_modules` 등 local file을 포함했다. | Git status와 claimed commit을 바꾸지 않고 local secret 또는 development artifact가 pack에 유출될 수 있었다. | `816f627`: pinned source commit의 `git archive`에서만 선택 경로를 복사하므로 tracked blob만 들어간다. Copied example directory 안의 ignored `.env` probe가 archive에 없음을 실제 확인함. |
| CONCURRENT-RELEASE-SNAPSHOT | Medium | Working-tree copy 사이에 다른 process가 변경을 commit하면 두 status check 모두 clean이어서 새 `HEAD`를 기록할 수 있었다. | Archive가 old content를 섞으면서 더 새로운 clean `HEAD`를 provenance로 주장할 수 있었다. | `816f627`: 추출 전에 source SHA를 고정하고 모든 file을 그 Git tree에서 가져오며, manifest 전 HEAD 동일성을 검사하고 pinned SHA만 기록함. |
| PROVER-HANDLER-DOC-ALIGNMENT | Low | `fc45edd` 뒤에도 한영 security/operations 문서 4쌍이 low-level prover handler에 body limit이 없다고 설명했다. | 실제보다 보수적이지만 현재 trust boundary와 달라 downstream review를 혼동시킬 수 있었다. | Publication record에서 raw handler도 hard cap을 유지하고 production wrapper가 auth, gzip wire/decompressed limit, health/readiness policy, server timeout을 추가로 담당한다고 정렬함. |
| INVALID-COMMIT-PROVENANCE | High | 최초 immutable record가 short commit `816f627`을 존재하지 않는 40-character object ID로 잘못 확장했다. | Exact validation range가 resolve되지 않아 publication claim을 재현할 수 없고 publication gate provenance가 무효였다. | 이 record에서 실제 object `494c72df2cad38dc1cc97d5e6e0f15b38e0c82d2`를 모든 exact range에 고정하고 공개 전에 range를 실행함. |
...
| RELEASE-PACK-COMPLETENESS | Medium | Git-blob verify가 archive에 남은 file만 순회해 non-required tracked source file을 삭제하고 checksum을 다시 만들면 통과했다. | Expected commit을 주장하는 불완전 handoff pack이 승인될 수 있었다. | `3453b55`: generator/verifier가 tracked selected-path manifest를 공유하고 recursive expected Git file set과 archive file set의 exact equality를 요구한다. JS prover source 삭제 variant가 실패함. |
| CANONICAL-RELEASE-MANIFEST | Medium | Generated `RELEASE-MANIFEST.txt`는 commit line만 검사했다. | Source identity, contents description, validation instruction을 수정하고 self-checksum할 수 있었다. | `3453b55`: tracked manifest template을 canonical version/commit/time으로 render하고 generated manifest 전체를 byte-for-byte 비교함. |
| SAFE-TAR-VALIDATION | Medium | Tar type/duplicate 검사를 extraction 뒤 filesystem에서 수행해 뒤의 regular duplicate가 앞의 symlink entry를 숨길 수 있었다. | Tar reader마다 다른 effective content를 소비하는 archive가 승인될 수 있었다. | `3453b55`: bounded Python `tarfile` 검사로 non-canonical path, duplicate header, link/special entry, multiple root, size/member overflow를 file acceptance 전에 거부함. |
| IMMUTABLE-EXPECTED-COMMIT | Medium | `RELEASE_PACK_EXPECTED_COMMIT`이 moving ref와 short SHA를 허용했다. | Branch/tag/`HEAD`가 이동해 immutable out-of-band trust anchor가 아니게 될 수 있었다. | `3453b55`: explicit verify는 canonical lowercase 40-character commit SHA만 허용하고 그 exact local commit을 요구함. |
...
## Historical — Independent Golden Known-Answer Test
...
## Historical — Property와 Fuzz Coverage
...
## Historical — Fresh Localnet과 Recovery 결과
...
| `make release-pack` | 독립 공개 검증 한영 보고서를 포함해 통과 |
...
## Historical — Accepted Residual과 Production TODO

| Residual | Owner | Experimental 공개에서 수용하는 이유 | Production blocking |
| --- | --- | --- | --- |
| External ZK/constraint audit, final source freeze, official MPC/trusted setup, transcript/toxic-waste evidence | Protocol/release owner | 독립 공개 검증 명시적 비범위이며 development artifact만 사용함 | Yes |
