...
- `x/privacy/client/sdk/conformance/testdata/privacy_batch_joinsplit_v1_contract.json` freezes the future 16/32 capacities, canonical 1..64-byte `audit_key_id` grammar `[a-z0-9][a-z0-9._-]*`, vector roots, effect ID, exact canonical owner-effect digest, corrected max-shape wire-state values, and the 12 public inputs in this exact order: `MerkleRoot`, `ChainDomainHi`, `ChainDomainLo`, `ExpiresAtUnix`, `InputCount`, `OutputCount`, `NullifierRoot`, `CommitmentRoot`, `UserDisclosureRoot`, `FullDisclosureRoot`, `PayloadDigestHi`, `PayloadDigestLo`. The independent payload SHA-256 is `f2588c...24b0`; the max canonical payload is `65,384` bytes. The wire goldens are Tx `65,294` bytes, typed scan KV `75,105` bytes, total KV write `173,409` bytes, and query response `74,551` bytes.
...
Current-root path queries use incremental nodes and are not subject to the 1,048,576-leaf rebuild cap. Every non-current historical path requires persisted `(root, leaf_count, height)` metadata and is capped at 1,048,576 leaves because its nodes are rebuilt. Above the cap, use the current root or a trusted local historical-path index. A complete persisted per-prefix snapshot metadata index still permits genesis export above the cap without rebuilding all historical nodes.

`BatchJoinSplit16x32` and `batch_feasibility.proto` are Session 2 feasibility artifacts, not a production batch message, circuit artifact, keeper handler, or prover route. The corrected full-shape reference gate measured `1,111,837` constraints, peak RSS `3,339,862,016` bytes, `55.892 ms/output` max-shape warm proving, and `2.789x` per-output improvement over native 2x2. Artifact consumers must pin `privacy-note-v1`; validators use exact consensus identity and required VKs, while provers lazily load selected R1CS/PK pairs. Reference prover admission defaults are one in-flight, four queued, and a positive 8 MiB request limit per circuit/service boundary.
