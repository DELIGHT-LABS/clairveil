...
- `x/privacy/types/disclosure_blinding.go`, `x/privacy/client/sdk/transfer/payload.go`: secret-free `DBS_*` error contract 정의, separated 2x2 blinding 생성, proving 전 prepared reuse 거부, output/disclosure/ciphertext 확정 후 single owner signature 생성
...
- Batch는 output slot별 `DISCLOSURE-BLINDING-SEPARATION`을 circuit에서 강제합니다. Production 2x2도 같은 output 0 relation을 circuit, native/prepared validator와 structured pre-sign boundary에서 강제하고 JoinSplit development R1CS/PK/VK identity를 회전했습니다. `DISCLOSURE-BLINDING-SEPARATION`과 security, protocol, chain-core, client-integration gates의 fresh closure는 완료됐습니다. SDK-wide cross-output/input freshness는 별도의 더 강한 정책이며 `BATCH-SIGNER-SECRET-FRESHNESS`이 structured-signer enforcement를 닫았고 구현은 signature release 전에 non-canonical BN254 alias도 거부하도록 보강됐습니다. 독립 공개 검증은 이 동작을 확인하고 unresolved security-relevant Medium 0건으로 PASS해 experimental source publication을 승인했습니다.
...
- Role-aware loader는 불필요한 key 노출을 줄이지만 exact consensus identity는 계속 mandatory입니다. Reference admission bound는 circuit별 in-flight 1개와 queued 4개, positive 8 MiB body limit이며 0은 invalid입니다. Raw transport handler도 hard body cap은 유지하지만 service-level bearer auth, gzip wire/decompressed limit, health/readiness policy, server timeout은 우회합니다.
...
Batch reference Go SDK, bounded remote prover, typed scanner, one-proof payroll graph, batch CLI/tutorial은 이제 이 repository threat boundary 안에 있습니다. Downstream JS/TS product, formal setup, external audit, production artifact custody/distribution은 여전히 boundary 밖입니다. Raw handler나 ad hoc downstream variant를 노출하면 필요한 review 없이 private-witness/compatibility boundary를 넓히게 됩니다.
