...
- Batch는 output slot별 `DISCLOSURE-BLINDING-SEPARATION`을 circuit에서 강제합니다. 2x2 native/prepared boundary도 같은 계약을 사용하지만 production R1CS/VK와 structured signer enforcement는 Session 3A pending입니다. SDK-wide cross-output/input freshness는 별도의 더 강한 정책이며 batch structured-signer gap `G3B-04`는 유지됩니다.
