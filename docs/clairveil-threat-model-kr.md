...
- Batch는 output slot별 `DISCLOSURE-BLINDING-SEPARATION`을 circuit에서 강제합니다. Production 2x2도 같은 output 0 relation을 circuit, native/prepared validator와 structured pre-sign boundary에서 강제하고 JoinSplit development R1CS/PK/VK identity를 회전했습니다. `S4-B02`와 Gate 1/2/3A fresh closure는 완료됐고 Session 3B re-entry는 UNBLOCKED입니다. SDK-wide cross-output/input freshness는 별도의 더 강한 정책이며 batch structured-signer gap `G3B-04`가 남아 Gate 3B/publication은 BLOCKED입니다.
