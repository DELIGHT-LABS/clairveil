...
## Batch Chain Core / Wallet Integration Boundary
...
Do not treat batch chain-core support as wallet support. The existing batch-oriented payroll UI still submits current 2x2 operations. The downstream wallet integration must use the production 12 public inputs (`MerkleRoot`, `ChainDomainHi`, `ChainDomainLo`, `ExpiresAtUnix`, `InputCount`, `OutputCount`, `NullifierRoot`, `CommitmentRoot`, `UserDisclosureRoot`, `FullDisclosureRoot`, `PayloadDigestHi`, `PayloadDigestLo`) and remain feature-gated until the reference builder, prover route, decryption, submission, and reconciliation components are integrated and the complete flow passes end to end.
