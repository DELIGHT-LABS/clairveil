...
## Session 2 Wallet Foundation Boundary
...
Persist the unified cursor `(height, global_sequence, output_index)` atomically and derive spend paths from a snapshot for exactly the selected root. A current-root incremental path has no 1,048,576-leaf cap. A non-current historical path uses persisted root/count/height metadata, but its nodes are rebuilt under a 1,048,576-leaf bound; above that cap, use the current root or a trusted local historical index. Remote historical lookup can disclose the treasury's timing and state interest, so retain that privacy warning in provider selection. A canceled remote proving request does not guarantee that the in-process solver stopped; keep the affected reservations until job/chain reconciliation. Automatic prover failover stays disabled.

Do not treat the Session 2 `BatchJoinSplit16x32` prototype as wallet support. The existing batch-oriented payroll UI still submits current 2x2 operations. Future UI/schema work may reserve the frozen 12 public inputs (`MerkleRoot`, `ChainDomainHi`, `ChainDomainLo`, `ExpiresAtUnix`, `InputCount`, `OutputCount`, `NullifierRoot`, `CommitmentRoot`, `UserDisclosureRoot`, `FullDisclosureRoot`, `PayloadDigestHi`, `PayloadDigestLo`) but must remain feature-gated until a production message, verifier artifact, keeper handler, and scanner integration exist.
