...
0 deposit 복구로 `DepositAuditFieldV1`의 non-zero 제약이 제거되어 기존 deposit R1CS/PK/VK는 호환되지 않습니다. 현재 source의 `clairveil-setup --out <new-directory> --development`로 전체 4회로 bundle과 manifest를 다시 생성하고, 새 manifest의 `CircuitSetIdentity`를 fresh genesis의 audit configuration에 반영해야 합니다. Circuit set 이름이 같아도 기존 key나 manifest를 재사용하지 말고 node/prover의 strict identity 검증을 통과해야 합니다. 이 절차는 development setup이며 production trusted setup을 대신하지 않습니다.
