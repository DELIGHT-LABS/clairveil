...
| [Prover HTTP client](../examples/js-sdk-prover-http-client/README.md) | V2 audit-field deposit mock with finite timeout, bearer auth, version/circuit/artifact checks and ordered PI23 binding. |

Run commands are maintained with each example and in the [testing guide](clairveil-testing-guide.md). These examples are reference consumers, not a production SDK. The HTTP demo uses a mock for `POST /v2/prover/audit-field` and rejects changed response bindings. It does not produce a complete proving witness or verify a Groth16 proof. Before constructing a live transaction, verify the proof against the exact local artifacts and final PI23, as `ValidateAuditFieldProofResponse` does. Port route-specific binding and fixture checks into downstream CI.

## 17. Batch Transfer Reference Addendum

The repository now includes the production core plus a reference Go batch builder, bounded proof adapter/HTTP route, decrypting typed scanner, durable payroll integration, and staged batch CLI. This JS SDK handoff still requires a downstream JS/TS implementation of those contracts. The legacy `transfer-batch` helper orchestrates V1 2x2 messages; current one-proof batch submission uses `clairveil.privacy.v2.MsgBatchTransfer`.
...
- Current `batch-joinsplit-16x32-audit-field-u128-v1` uses the 23 public inputs in `auditfield.PublicInputSchema()`, in that exact order. The retained inner `BatchJoinSplit16x32` relation's 12 inputs are not the live prover contract. Downstream JS/TS support remains feature-gated until it independently reproduces the current conformance fixtures and native V2 behavior.
- A downstream JS/TS batch builder must reproduce the V2 Aux/context/envelope binding in `batchtransfer.PrepareAuditV2FromPreparedAuditV2Payload` and follow the [current HTTP contract](clairveil-proverd-http-api.md). `AuditOutputsToAux` encodes output effects; SHA-256 with domain `clairveil.audit.aux.v1` supplies `AuxHi`/`AuxLo`, and the audit envelope supplies `CipherRoot0`/`CipherRoot1`. The retained `CanonicalBatchTransferPayloadBytesV1` is a legacy payload contract, not the V2 proof binding. Do not substitute protobuf marshal or JSON for canonical encoding.
