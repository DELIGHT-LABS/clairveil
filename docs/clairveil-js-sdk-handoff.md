...
The Go 2x2 boundary now uses `JoinSplitOwnerIntentSigningRequestV1`, which carries the canonical policy, recipient output randomness, user/full blindings, and final effect. `ValidateJoinSplitOwnerIntentSigningRequestV1` recomputes the domain, payload digest, and final intent before applying `DBS-01..03`; `SignValidatedJoinSplitOwnerIntentV1` never invokes the callback for an invalid request. A downstream structured wallet signer MUST preserve this fail-before-sign contract. `S4-B02` implementation is resolved without changing transfer payload `v5`, proof/request/response `v2`, NoteV1, fixed payload encoding, disclosure digest formulas, or the 13-input schema; the new JoinSplit VK identity is `3dd068d67137791666e81e599b8b3b6820f92d8aed8234eca16370b2d54ed112`.
