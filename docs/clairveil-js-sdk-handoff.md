...
## 17. Session 3A Core Addendum

The chain core now includes the production `BatchJoinSplit16x32` circuit descriptor, `MsgBatchTransfer`, keeper handler, minimal event, and typed `privacy-scan-v2` batch records. This JS SDK handoff has not implemented a batch builder, proof adapter/HTTP route, decrypting scanner UX, or payroll integration. The existing `transfer-batch` helper still orchestrates native 2x2 messages and is not one `MsgBatchTransfer` proof.
...
- The production `BatchJoinSplit16x32` public-input order is `MerkleRoot`, `ChainDomainHi`, `ChainDomainLo`, `ExpiresAtUnix`, `InputCount`, `OutputCount`, `NullifierRoot`, `CommitmentRoot`, `UserDisclosureRoot`, `FullDisclosureRoot`, `PayloadDigestHi`, `PayloadDigestLo`. Client support remains feature-gated until Session 3B completes its end-to-end path.
- A Session 3B batch builder must reproduce `CanonicalBatchTransferPayloadBytesV1` exactly: format `1`, `u32be` vector counts, `u32be(length) || bytes` for every byte field, output fields in proto declaration order, followed by audit ID/epoch/target and expiry. SHA-256 domain `clairveil.batch-transfer-payload.v1` is split into non-reduced 128-bit limbs. Only `creator` and `proof` are excluded. Do not invent protobuf-marshal, JSON, or sorted-field alternatives.
