...
## 9. NoteV1 And Session 3A Core Threat Delta
...
- Batch enforces `DISCLOSURE-BLINDING-SEPARATION` per output slot in-circuit. Production 2x2 enforces the same output-0 relation in the circuit, native/prepared validator, and structured pre-sign boundary, and the JoinSplit development R1CS/PK/VK identity has been rotated. `S4-B02` and fresh Gates 1/2/3A/3B are closed. SDK-wide cross-output/input freshness is a separate stronger policy; `G3B-04` closed its structured-signer enforcement and Session 4 added non-canonical BN254 alias rejection before signature release. Session 4 passed with no unresolved security-relevant Medium finding and approved experimental source publication.
