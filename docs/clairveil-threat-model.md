...
- Batch enforces `DISCLOSURE-BLINDING-SEPARATION` per output slot in-circuit. The 2x2 native/prepared boundary now uses the same contract, but its production R1CS/VK and structured signer enforcement are pending Session 3A. SDK-wide cross-output/input freshness is a separate stronger policy and the batch structured-signer gap remains `G3B-04`.
