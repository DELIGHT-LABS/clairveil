...
- Batch enforces `DISCLOSURE-BLINDING-SEPARATION` per output slot in-circuit. Production 2x2 enforces the same output-0 relation in the circuit, native/prepared validator, and structured pre-sign boundary, and the JoinSplit development R1CS/PK/VK identity has been rotated. `S4-B02` implementation is resolved, with fresh independent Gate 1/2/3A review still required. SDK-wide cross-output/input freshness is a separate stronger policy and the batch structured-signer gap remains `G3B-04`.
