...
Session 3B remains outside this boundary: there is no public batch Go SDK, remote batch prover route, wallet scanner/decrypt UX, one-proof payroll integration, or batch CLI/tutorial. Formal setup and production artifact custody/distribution are also absent. Exposing ad hoc versions of those interfaces would expand the private-witness and compatibility boundary without the required review.
