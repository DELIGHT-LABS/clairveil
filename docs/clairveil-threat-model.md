...
- `x/privacy/types/disclosure_blinding.go`, `x/privacy/client/sdk/transfer/payload.go`: define the secret-free `DBS_*` error contract, generate separated 2x2 blindings, reject prepared reuse before proving, and finalize outputs/disclosures/ciphertexts before one owner signature.
...
- Repository-generated artifacts, including BatchJoinSplit16x32, are development-only; no formal trusted setup, production artifact release, or external audit has been performed.
...
- Unified scan ordering is `(height, global_sequence, output_index)`. Partial cursor persistence can skip or duplicate outputs, and using a path from a different root invalidates the witness. Current-root paths read persisted incremental nodes and do not consume the online historical-rebuild budget. A missing cached current root fails closed with `FailedPrecondition` and requires offline repair; the query never rebuilds or writes cached tree state. A non-current historical path requires persisted root/count/height metadata; the public query admits at most 1,024 leaves and two concurrent rebuilds per keeper, otherwise it returns `ResourceExhausted`. Use the current root or a trusted local historical index above that online bound. The separate offline recovery/export bound remains `MaxMerkleRebuildLeaves` (1,048,576). Remote historical root/path requests also reveal wallet timing and state interest to the provider, so retain that privacy warning.
- `BatchJoinSplit16x32`, `MsgBatchTransfer`, and the keeper handler are live chain-core contracts. The circuit enforces exact active prefixes/sentinels, independent paths, owner/key constraints, active-only distinctness, conservation, disclosure roots, and one owner signature. The keeper re-derives the 12 public fields, checks global nullifier/commitment uniqueness and historical-root/capacity conditions, and fails closed on any mismatch.
- Batch enforces `DISCLOSURE-BLINDING-SEPARATION` per output slot in-circuit. Production 2x2 enforces the same output-0 relation in the circuit, native/prepared validator, and structured pre-sign boundary, and the JoinSplit development R1CS/PK/VK identity has been rotated. Implementation of `DISCLOSURE-BLINDING-SEPARATION` is complete, and the fresh security, protocol, chain-core, and client-integration gates are closed. SDK-wide cross-output/input freshness is a separate stronger policy; `BATCH-SIGNER-SECRET-FRESHNESS` closed its structured-signer enforcement, and the implementation added non-canonical BN254 alias rejection before signature release, as confirmed by independent publication validation. Independent publication validation passed with no unresolved security-relevant Medium finding and approved experimental source publication.
- The ante chain caps signed batch `Any.value` bytes at 128 KiB, including duplicate-singular-field encodings that decode to a smaller message. Cheap bounded message/proof framing then precedes deterministic `BatchGasModelV1` precharge; semantic validation, artifact load, and proof verification occur only after that charge. State writes occur only after proof success and are committed through one nested cache-context write, so injected scan failures and 2x2+batch or batch+batch nullifier conflicts roll back the whole effect.
- Batch ciphertext/disclosure bytes live once in `PrivacyScanOutputV2`. Minimal ABCI and indexed events expose effect ID, counts, roots, expiry, versions, relayer, and audit identity rather than duplicating payload bytes. Corrupt, orphan, non-adjacent, or version-mismatched typed records fail closed.
- The role-aware loader reduces unnecessary key exposure, but exact consensus identity remains mandatory. The reference admission bound is one in-flight plus four queued jobs per circuit and a positive 8 MiB body limit; zero is invalid. The raw transport handler retains the hard body cap but bypasses service-level bearer auth, gzip wire/decompressed limits, health/readiness policy, and server timeouts.
...
The live public-input order is `MerkleRoot`, `ChainDomainHi`, `ChainDomainLo`, `ExpiresAtUnix`, `InputCount`, `OutputCount`, `NullifierRoot`, `CommitmentRoot`, `UserDisclosureRoot`, `FullDisclosureRoot`, `PayloadDigestHi`, `PayloadDigestLo`, with schema SHA-256 `5606327d69dcb06c00811f2135291d39a2ea1cedf554f114f7eb4a178098d333`. Any change requires a new identity/schema and security review.

The batch reference Go SDK, bounded remote prover, typed scanner, one-proof payroll graph, and batch CLI/tutorial are now inside this repository threat boundary. A downstream JS/TS product, formal setup, external audit, and production artifact custody/distribution remain outside it. Exposing raw handlers or ad hoc downstream variants would still expand the private-witness and compatibility boundary without the required review.
