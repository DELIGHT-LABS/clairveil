...
- Batch enforces `DISCLOSURE-BLINDING-SEPARATION` per output slot in-circuit. Production 2x2 enforces the same output-0 relation in the circuit, native/prepared validator, and structured pre-sign boundary, and the JoinSplit development R1CS/PK/VK identity has been rotated. `S4-B02` and fresh Gates 1/2/3A are closed, and Session 3B re-entry is unblocked. SDK-wide cross-output/input freshness is a separate stronger policy and the batch structured-signer gap remains `G3B-04`, so Gate 3B and publication remain blocked.
