# Clairveil Privacy Circuit 공개 벤치마크 계획
...
현재 상태: 구현됨. 다만 공개 기준으로는 반복 횟수, benchstat, 환경 메타데이터 자동 수집이 더 필요합니다.
...
go test ./x/privacy/circuit \
  -run '^$' \
  -bench 'Benchmark(Deposit|Spend|JoinSplit)CircuitProve' \
  -benchmem \
  -count=10
```
...
go test ./x/privacy/circuit \
  -run '^$' \
  -bench 'Benchmark(Deposit|Spend|JoinSplit)Circuit(Verify|PublicWitness)' \
  -benchmem \
  -count=10
```
...
- deposit/transfer/withdraw/self-merge/dummy-deposit별 예상 fee가 계산됨
