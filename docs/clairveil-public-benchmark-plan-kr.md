...
- `cmd/clairveil-localnetload`와 `make privacy-localnet-tps-bench`가 tx metrics를 `chain_tps` structured summary로 변환해 submitted/accepted/included/successful tx/sec, failed tx rate, inclusion latency, gas metric을 기록합니다.
...
현재 상태: 하네스 구현 완료. `make privacy-bench-localnet`가 e2e smoke tx query에서 gas/success/height/timestamp metric을 추출하고 fee report, reserve invariant snapshot, localnet structured summary를 생성합니다. `cmd/clairveil-localnetload`와 `make privacy-localnet-tps-bench`는 tx metrics를 `chain_tps` row로 변환해 `submitted_tx/sec`, `accepted_tx/sec`, `included_tx/sec`, `successful_tx/sec`, `failed_tx_rate`, `inclusion_latency_ms`, `gas_used`를 `benchmarks/privacy-localnet-tps` report에 포함합니다. Public Chain TPS claim 승격에는 대규모 open-loop account pool과 sequence contention 없는 batch submission을 downstream/staging 환경에서 같은 bucketed tx metrics schema로 feed해야 합니다.
