...
현재 `privacy-localnet-tps-bench` wrapper는 e2e smoke flow를 계측 가능한 TPS report로 승격하는 경로입니다. Public chain TPS claim에는 최소 2개 이상의 target tx/sec bucket, before/after reserve snapshot, chain config evidence, saturation profile evidence가 필요합니다. 대규모 open-loop account pool/sequence contention 없는 batch submission은 downstream/staging 환경에서 같은 bucketed tx metrics schema로 feed합니다.
...
현재 상태: 하네스 구현 완료. `make privacy-bench-localnet`가 e2e smoke tx query에서 gas/success/height/timestamp metric을 추출하고 fee report, reserve invariant snapshot, localnet structured summary를 생성합니다. `cmd/clairveil-localnetload`와 `make privacy-localnet-tps-bench`는 tx metrics를 `chain_tps` row로 변환해 `submitted_tx/sec`, `accepted_tx/sec`, `included_tx/sec`, `successful_tx/sec`, `failed_tx_rate`, `inclusion_latency_ms`, `gas_used`를 `benchmarks/privacy-localnet-tps` report에 포함합니다. Tx별 `success`가 누락된 입력은 failed tx로 계산합니다. Public Chain TPS claim 승격에는 대규모 open-loop account pool과 sequence contention 없는 batch submission을 downstream/staging 환경에서 같은 bucketed tx metrics schema로 feed해야 합니다.
...
현재 상태: native 하네스 구현 완료. Native CLI smoke/reference user latency는 별도 result family로 생성할 수 있습니다. Browser/WASM adapter가 없는 동안 browser user latency public claim은 `browser_adapter_ready=true` evidence를 만족할 수 없으므로 승격 대상이 아닙니다. Remote prover user latency는 같은 trace schema로 ingest할 수 있지만, 현재 CLI smoke runner가 remote proof 경로를 직접 실행하지는 않습니다. Remote public claim은 실제 remote client trace와 eligible `privacy-proverd-load` report를 `CLAIM_LINKED_PROVER_REPORT_FILE`로 연결해야 합니다.
