...
Sender self-view disclosure는 sender가 자신의 보낸 transfer 상세를 나중에 복구하기 위한 기본 기능입니다. Client는 기본적으로 포함하고, 사용자가 명시적으로 끌 수 있게 해야 합니다.

주요 결정:

- On-chain event에는 self-view target pubkey를 넣지 않습니다. Static disclosure pubkey를 노출하면 sender clustering 단서가 됩니다.
- `self_view_disclosure_digest`와 `self_view_disclosure_payload`만 저장하고, sender disclosure private key로 trial decrypt합니다.
- Self-view를 끄면 tx size는 줄지만, client가 local cache를 잃었을 때 보낸 transfer 상세 복구가 제한됩니다.
- Sender disclosure private key가 유출되면 self-view payload가 있는 보낸 transfer 상세가 노출될 수 있습니다.
...
- public disclosure, recipient disclosure, sender self-view disclosure, audit disclosure를 구분합니다.
...
- sender self-view를 끄는 옵션을 UI에 어떻게 노출할 것인가?
