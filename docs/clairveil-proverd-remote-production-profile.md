...
`proverservice.DefaultRuntimeInfo()` describes the complete four-route reference daemon. The bounded service handler derives the `routes` and `circuits` reported by `/healthz` and `/readyz` from the non-nil provers in its configured `provertransport.ProverSet`; compatibility constructors with a partial prover set therefore do not advertise unconfigured routes. `NewReferenceHandler` configures all four provers and reports the complete reference inventory.
...
The production 16x32 public schema uses this exact order: `MerkleRoot`, `ChainDomainHi`, `ChainDomainLo`, `ExpiresAtUnix`, `InputCount`, `OutputCount`, `NullifierRoot`, `CommitmentRoot`, `UserDisclosureRoot`, `FullDisclosureRoot`, `PayloadDigestHi`, `PayloadDigestLo`. Its existence does not authorize an ad-hoc remote endpoint; use only the bounded route provided by the batch integration, with admission, authentication, and end-to-end conformance intact.
