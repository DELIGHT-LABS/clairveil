...
Unified cursor `(height, global_sequence, output_index)` 전체를 atomically 저장하고 선택한 root와 정확히 같은 snapshot에서 spend path를 구합니다. Current-root incremental path에는 1,048,576-leaf cap이 없습니다. Non-current historical path는 persisted root/count/height metadata를 사용하지만 node를 bounded rebuild하므로 1,048,576 leaves로 제한됩니다. Cap을 넘으면 current root 또는 trusted local historical index를 사용합니다. Remote historical lookup은 treasury timing과 관심 state를 노출하므로 provider 선택 UI에서 이 privacy warning을 유지합니다. Canceled remote proving request가 in-process solver 중단을 보장하지 않으므로 job/chain reconciliation 전까지 관련 reservation을 유지합니다. Automatic prover failover는 계속 비활성화합니다.

Session 2 `BatchJoinSplit16x32` prototype을 wallet support로 취급하지 않습니다. 기존 batch-oriented payroll UI는 여전히 현재 2x2 operation을 submit합니다. Future UI/schema 작업은 동결된 12개 public input(`MerkleRoot`, `ChainDomainHi`, `ChainDomainLo`, `ExpiresAtUnix`, `InputCount`, `OutputCount`, `NullifierRoot`, `CommitmentRoot`, `UserDisclosureRoot`, `FullDisclosureRoot`, `PayloadDigestHi`, `PayloadDigestLo`)을 reserve할 수 있지만 production message, verifier artifact, keeper handler, scanner integration이 생길 때까지 feature-gated 상태를 유지합니다.
