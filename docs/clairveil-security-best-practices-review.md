...
### 6.1 Deposit Prover Boundary

Treat `POST /v1/prover/deposit` as a bounded raw-handler/auth/redaction surface, not a special client shortcut. It accepts only versioned circuit witness data and must use the service wrapper for bearer auth, gzip and body limits, admission, readiness, timeouts, secret-free errors, and `no-store`; request/response bodies and witness values are never logged. Recommend `Content-Type: application/json`, retain omission compatibility for existing `v1` clients, reject unsupported supplied media types with `415`, return `Allow: POST` on `405`, use `400` before proving, and use `500` after validated prover invocation. See the [general HTTP API](clairveil-proverd-http-api.md) and [deposit API](clairveil-proverd-deposit-api.md).
