...
- The required contracts are transfer payload `v5`, transfer proof/request/response `v2`, withdraw prover/final payload and proof/request/response `v2`, and disclosure plaintext/query version `privacy-fixed-v1`. Validate these exact versions before proving or signing.
...
10. Accept only validated `v5` transfer and `v2` withdraw prepared payloads, preserve the exact `SpendIntentV2`/`TransferIntentV2` public-input order, and pin the complete `privacy-note-v1` artifact set.
...
The exact shared blinding contract is per output slot, not an unqualified global pairwise claim: `DBS-01` gates user-vs-output randomness on enabled non-all-private disclosure; `DBS-02` and `DBS-03` keep full-vs-output and full-vs-user active for every enabled output; all-private user blinding is zero; disabled policy/randomness/blindings are all zero. Batch already enforces 96 gated inequality sites. JoinSplit2x2 applies only to recipient output `0`; output `1`, input randomness, and cross-output reuse are outside these three circuit relations. The stronger SDK global-freshness/structured-signer requirement remains a separate boundary.
...
The batch integration provides experimental reference Go surfaces for the one-proof batch planner/preparer, bounded remote HTTP prover route, typed scanner/decrypt flow, durable payroll graph, and staged CLI/tutorial. ClairveilJS 0.3.1 implements the same prepared effect for Cosmos `MsgBatchTransfer` and EVM `singleProofBatchTransfer`. Never expose the raw transport handler, and treat every prover request as highly sensitive witness data. Deposit CLI output must not print `NotePlaintextV1` or randomness.
