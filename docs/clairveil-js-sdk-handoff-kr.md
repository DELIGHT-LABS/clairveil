...
| [Prover HTTP client](../examples/js-sdk-prover-http-client/README-kr.md) | V2 audit-field deposit mock의 finite timeout, bearer auth, version/circuit/artifact 검사와 순서가 고정된 PI23 binding. |

실행 명령은 각 예제와 [테스트 가이드](clairveil-testing-guide-kr.md)에서 관리합니다. 예제는 production SDK가 아닌 reference consumer입니다. HTTP demo는 `POST /v2/prover/audit-field` mock으로 response binding 변경을 거부하는지 확인합니다. 완전한 proving witness를 만들거나 Groth16 proof를 검증하지는 않습니다. Live transaction을 구성하기 전에는 `ValidateAuditFieldProofResponse`처럼 exact local artifact와 final PI23으로 proof를 검증합니다. Downstream CI에 route-specific binding과 fixture 검증을 이식합니다.

## 17. Batch transfer reference addendum

Repository에는 production core와 reference Go batch builder, bounded proof adapter/HTTP route, decrypting typed scanner, durable payroll integration, staged batch CLI가 포함됩니다. 이 JS SDK handoff에는 해당 contract의 downstream JS/TS 구현이 여전히 필요합니다. Legacy `transfer-batch` helper는 V1 2x2 message를 orchestration하며 current one-proof batch submission은 `clairveil.privacy.v2.MsgBatchTransfer`를 사용합니다.
...
- Current `batch-joinsplit-16x32-audit-field-u128-v1`은 `auditfield.PublicInputSchema()`의 public input 23개를 정확히 같은 순서로 사용합니다. 보존된 내부 `BatchJoinSplit16x32` relation의 input 12개는 live prover 계약이 아닙니다. Downstream JS/TS support는 current conformance fixture와 native V2 동작을 독립 재현하기 전까지 feature-gated 상태입니다.
- Downstream JS/TS batch builder는 `batchtransfer.PrepareAuditV2FromPreparedAuditV2Payload`의 V2 Aux/context/envelope binding을 재현하고 [현재 HTTP 계약](clairveil-proverd-http-api-kr.md)을 따라야 합니다. `AuditOutputsToAux`는 output effect를 인코딩하고, domain `clairveil.audit.aux.v1`의 SHA-256이 `AuxHi`/`AuxLo`를, audit envelope가 `CipherRoot0`/`CipherRoot1`을 결정합니다. 보존된 `CanonicalBatchTransferPayloadBytesV1`은 legacy payload 계약이며 V2 proof binding이 아닙니다. Canonical encoding을 protobuf marshal이나 JSON으로 대체하면 안 됩니다.
