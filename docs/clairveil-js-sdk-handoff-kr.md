...
`MsgBatchTransfer`는 1..16 input과 1..32 output을 proof 하나로 처리합니다. `MsgWithdraw`는 exact-match withdraw 메시지이며 output note 필드를 갖지 않습니다.
...
- `PrivacyScan`: full `(height, global_sequence, output_index)` cursor 기반 typed wallet projection으로 deposit/transfer/batch output을 스캔합니다.
- `PrivacyEvents`: 운영 진단용 raw deposit/transfer event feed를 읽습니다.
...
- `CheckNullifiers`: 여러 note의 spent 상태를 한 번에 갱신합니다. 일반 batch에는 POST JSON body binding을 쓰고 요청당 1000개로 chunk하며 GET은 작은 운영 진단에만 사용합니다.
...
1. `PrivacyScan(after, output_limit, event_limit)`으로 typed deposit/transfer/batch output projection을 가져옵니다.
2. 각 output의 `cipher_text`, `commitment`, `output_index`, `view_tag`, disclosure field와 transaction identity를 읽습니다.
3. Projection을 소비하기 전에 `scan_format_version`, `view_tag_version`을 검증합니다. 지원하지 않는 version이면 cursor를 전진시키지 않고 중단합니다.
...
5. `view_tag`는 untrusted optimization으로만 취급합니다. 없거나 형식이 틀리면 full trial decrypt를 수행합니다. Mismatch output을 건너뛰는 동작은 recovery 또는 forced rescan을 갖춘 명시적 fast mode 정책이어야 합니다.
6. Wallet root seed와 viewing key로 복호화를 시도하고, 필요한 경우 Go SDK와 같은 spend-key recovery path를 사용합니다.
7. 복호화에 성공한 note만 wallet DB에 저장합니다.
8. note commitment와 nullifier를 추적합니다.
9. `CheckNullifiers`로 spent 상태를 요청당 1000개 이하 batch로 갱신하고, 진단·복구에는 `CheckNullifier`를 사용합니다.
10. Rollback/reorg 대응을 위해 event height, global sequence, output index, tx hash를 함께 저장합니다.

`PrivacyScan`은 `scan_format_version=2`, `view_tag_version=1`과 다음 full cursor를 반환합니다. Client는 page의 모든 output을 검증·저장한 뒤 `(height, global_sequence, output_index)` cursor를 atomically commit합니다. Raw event query는 운영 진단에만 사용합니다.
...
Transfer는 payload `v5`와 proof/request/response `v2` 단일 contract를 사용합니다.
...
- Transfer payload는 `v5`, transfer proof와 prover request/response는 `v2`를 사용하며 다른 version은 거부합니다.
...
- `MsgWithdraw`는 output note 필드가 없는 exact-match operation입니다.
...
- Withdraw prover payload, proof, final payload, prover request/response, relay schema/handoff는 모두 `v2`이며 다른 version은 거부합니다.
...
Repository에는 production core와 reference Go batch builder, bounded proof adapter/HTTP route, decrypting typed scanner, durable payroll integration, staged batch CLI가 포함됩니다. ClairveilJS 0.3.1은 이 contract를 Cosmos와 EVM transport에서 구현하며 동일한 conformance fixture와 operation-evidence 규칙을 사용합니다.

SDK 구현에는 아래 current rule을 normative하게 적용합니다.
...
- Fresh genesis에서 exact artifact를 생성하고 empty note, scan cursor, reservation, prepared/proof namespace를 full typed scan으로 초기화합니다.
...
- Production `BatchJoinSplit16x32` public-input 순서는 `MerkleRoot`, `ChainDomainHi`, `ChainDomainLo`, `ExpiresAtUnix`, `InputCount`, `OutputCount`, `NullifierRoot`, `CommitmentRoot`, `UserDisclosureRoot`, `FullDisclosureRoot`, `PayloadDigestHi`, `PayloadDigestLo`입니다. ClairveilJS 0.3.1은 conformance fixture를 재현하고 Cosmos `MsgBatchTransfer`와 EVM `singleProofBatchTransfer`에서 같은 prepared effect를 사용합니다. Product UI 노출은 server feature gate와 transport capability를 모두 요구합니다.
