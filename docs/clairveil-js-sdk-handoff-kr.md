...
Chain core는 production `BatchJoinSplit16x32` circuit descriptor, `MsgBatchTransfer`, keeper handler, minimal event, typed `privacy-scan-v2` batch record를 포함합니다. 이 JS SDK handoff에는 batch builder, proof adapter/HTTP route, decrypting scanner UX, payroll integration이 아직 없습니다. 기존 `transfer-batch` helper는 native 2x2 message를 orchestration할 뿐 하나의 `MsgBatchTransfer` proof가 아닙니다.
...
- Production `BatchJoinSplit16x32` public-input 순서는 `MerkleRoot`, `ChainDomainHi`, `ChainDomainLo`, `ExpiresAtUnix`, `InputCount`, `OutputCount`, `NullifierRoot`, `CommitmentRoot`, `UserDisclosureRoot`, `FullDisclosureRoot`, `PayloadDigestHi`, `PayloadDigestLo`입니다. Session 3B end-to-end path가 완료되기 전까지 client support는 feature-gated 상태입니다.
- Session 3B batch builder는 `CanonicalBatchTransferPayloadBytesV1`을 exact하게 재현해야 합니다. Format `1`, `u32be` vector count, 모든 byte field의 `u32be(length) || bytes`, proto 선언 순서의 output field, audit ID/epoch/target, expiry 순서입니다. SHA-256 domain `clairveil.batch-transfer-payload.v1`을 non-reduced 128-bit limb 둘로 나눕니다. `creator`와 `proof`만 제외하며 protobuf marshal, JSON, sorted-field 대안을 만들면 안 됩니다.
