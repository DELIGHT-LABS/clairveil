...
Payroll reference는 deposit/native-2x2-transfer/withdraw surface를 계속 사용합니다. Production consensus circuit, `MsgBatchTransfer`, keeper handler, typed scanner contract는 존재하지만 payroll batch builder, prover route, execution/reconciliation path, UX는 아직 없습니다. 기존 payroll `transfer-batch` flow는 현재 2x2 message를 coordination하며 하나의 16x32 proof가 아닙니다.
