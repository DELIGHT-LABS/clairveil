...
Payroll reference는 현재 production deposit/native-2x2-transfer/withdraw surface를 계속 사용합니다. Session 2의 `BatchJoinSplit16x32` protobuf와 circuit은 feasibility prototype이자 동결된 future contract입니다. Production consensus circuit, keeper handler, scanner contract가 제공되기 전에는 batch message, prover route, payroll execution path를 추가하지 않습니다. 기존 payroll `transfer-batch` flow는 현재 2x2 message를 coordination하며 하나의 16x32 proof가 아닙니다.

새 JS 구현은 `privacy-note-v1`을 pin하고 canonical `privacy-fixed-v1` note/disclosure/typed-envelope byte를 사용하며 raw ciphertext나 legacy JSON plaintext를 거부해야 합니다. 모든 32-byte asset ID는 authoritative `AssetRegistryV1`으로 resolve하고 payroll denom configuration과 registry result가 일치해야 합니다. Fresh genesis로만 migrate합니다. Note reservation, cached note/cursor, prepared/proof job, development artifact를 지운 뒤 다시 생성하고 rescan합니다.

Unified scan cursor `(height, global_sequence, output_index)` 전체와 same-root Merkle path snapshot을 사용합니다. Current-root incremental path에는 1,048,576-leaf cap이 없습니다. Non-current historical path는 persisted root/count/height metadata를 사용하지만 node를 bounded rebuild하므로 1,048,576 leaves로 제한됩니다. Cap을 넘으면 current root 또는 trusted local historical index를 사용합니다. Remote historical path query는 treasury activity를 드러내므로 privacy warning을 유지하고 필요하면 privacy-preserving infrastructure를 사용합니다. Future batch schema 작업은 `MerkleRoot`, `ChainDomainHi`, `ChainDomainLo`, `ExpiresAtUnix`, `InputCount`, `OutputCount`, `NullifierRoot`, `CommitmentRoot`, `UserDisclosureRoot`, `FullDisclosureRoot`, `PayloadDigestHi`, `PayloadDigestLo` 순서를 정확히 보존해야 하지만 live 기능처럼 advertise하면 안 됩니다.
