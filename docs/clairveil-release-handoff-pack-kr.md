...
13. Batch integration은 public `MsgBatchTransfer` Go SDK, wallet scanner/decrypt path, one-proof payroll workflow, batch CLI/tutorial, ClairveilJS 0.3.1 Cosmos/EVM batch API를 제공합니다. Example WebApp은 explicit server feature gate 뒤에서만 batch editor를 노출하고 encrypted prepared-artifact recovery, wallet confirmation, typed input/item/audit reconciliation을 요구합니다. SDK나 prover route가 있다는 사실만으로 product capability를 발견하면 안 됩니다.
14. EVM에서는 `evmNativeDenom`, `payable-exact-value` deposit binding, canonical precompile address, receipt/finality evidence, outer scan tx hash와 Ethereum tx hash의 indexed 관계를 검증합니다. Optional EIP-712 authorization, contract/privacy-state adapter, custom finality policy는 profile-scoped reviewed dependency로 취급하고 end-to-end test합니다.
...
| Batch chain-core/client-integration boundary | Circuit, `MsgBatchTransfer`, keeper, deterministic gas, typed scan/minimal event, genesis, reference Go SDK/prover/scanner/payroll/CLI, ClairveilJS 0.3.1 Cosmos/EVM API와 feature-gated example flow는 구현됐고 formal setup, production artifact delivery, target-chain product acceptance는 별도임 | 선택한 chain, prover, encrypted recovery, wallet confirmation, typed reconciliation, Cosmos/EVM product E2E가 함께 통과할 때까지 batch gate를 끕니다. |
...
9. 모든 팀이 Go reference, ClairveilJS 0.3.1 Cosmos/EVM batch surface, feature-gated example flow는 구현됐지만 target-chain product acceptance, formal trusted setup, production artifact distribution은 미완료임을 기록했습니다.
