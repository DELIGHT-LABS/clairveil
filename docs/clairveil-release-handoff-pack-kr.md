...
12. Vendored ClairveilJS 0.2.0 package는 `privacy-fixed-v1` fixture와 V5/V2 transfer/withdraw contract를 구현합니다. Pre-0.2 note cache, reservation state, relay recovery metadata는 fresh-genesis boundary로 취급해야 하므로 폐기하거나 별도 namespace로 분리한 뒤 재스캔하며 compatibility decoder를 추가하지 않습니다.
...
| External ClairveilJS compatibility | Vendored 0.2.0 package는 `privacy-fixed-v1`과 V5/V2 prepared-payload/proof contract를 구현하며 pre-0.2 browser persistence는 호환되지 않음 | 새 cache/reservation/relay namespace에서 재스캔하고 compatibility decoder를 추가하지 않습니다. Prepared transfer payload `v5`는 별도의 outer version으로 계속 유효합니다. |
...
9. 모든 팀이 batch reference integration Go reference user-facing batch surface는 구현됐고 downstream JS/web product delivery, formal trusted setup, production artifact distribution은 미완료임을 기록했습니다.
